...
5.4 Sorting Codelist Values ................................ ................................ ................................ ......................... 60 
...
15.1 Multiline Displayable Attributes  ................................ ................................ ................................ .... 186 
...
18.5.1  Creating report template using JasperSoft Studio ................................ ................................ .........................  230 
...
26.6.1  Server based installation  ................................ ................................ ................................ ...............................  332 
...
28.2.3  Raster layer with hierarchical structure from internal data source (basic formats tiff, jpeg, png)  ................  351 
 ...
 Configuration of folder hierarchy for source files  ................................ ................................ ................  352 
...
icons.  
• option.xm l – Defines optional system functionality, i.e. copying features, deriving borders, 
...
translationsArray and by the row order in each file).  14 2 – GLOBAL METADATA PROPERTIES  
...
Implementation Guide             </ber:featureAttributeArray>  
...
• maxLength  – Maximum number of units of length. Accepted values are 1 to 3276 7, default value 
...
• expression – expressi on can be used instead of format . The expression string can also include 
...
Example:  Another example of sim ple derived attribute Cable length. Here expression  is used instead of 
  ...
  <ber:attributeQuery>  
    <ber:simpleQuery>  
      <ber:attributeArray>  
        ...
        <ber:attribute xpath="at_length_of_ducts"/>  IMPLEMENTATION GUIDE    VERSION 8.24 41 
 
 
 
  Implementation Guide        </ber:attributeArray>  
      <ber:expression>NVL({0},0)+ NVL({1},0)</ber:expression>  
    </ber:simpleQuery>  
  </ber:attributeQuery>  
</ber:derivedAttribute>  
 Format element  
Generally a format element contains a text with one or more placeholders for ref erred attribute values. 
...
D or d  Decimal (Whole 
number)  {0:d}  FormatException  -12400  
E or e  Scientific  {0:e}  1.420000e+00  -1.240000e+04  
F or f  Fixed point  {0:f}  1.420000  -12400.000000  
G or g  General  {0:g}  1.42000  -14200.0  
X or x  Hexadecimal  {0:x4}  FormatException  cf90 
Number rounding  
Format  Value  Output  Comment  
{0:F2}  12345.6789  12345.68  rounds up  
{0:F0}  12345.6789  12346  rounds up  
{0:F6}  12345.6789  12345.678900   42 4 – ATTRIBUTES  
 
 
 
Implementation Guide       Format  Value  Output  Comment  
-&gt;{0,10:F2} &lt;- 12345.6789  ->··12345.68< - rounds up  
-&gt;{0,-10:F0} &lt;- 12345.6789  ->12346·····< - rounds up  
4.5.2 Attributes derived from r elated features  
...
assure, that just one value is returned. Other wise more values would be returned and it wouldn’t be 
...
typically NULL.  To return some reasonable result, fa llbackValue attribute has to be defined.  This works 
for derived attributes, not for virtu al attributes.  
...
Parcel count derives number of parcels crossed by a pipeline segment. There is a rel ation between 
...
<ber:derivedAttribute id="at_5012100_countparc" name="Parcel count"    
  dbName="COUNT_PARC">  
  <ber:dataType>  
    <ber:decimal precision="5" scale ="0"/> IMPLEMENTATION GUIDE    VERSION 8.24 43 
...
Example:  Use of an optional element <filterRelated>  within <relationQuery>  element. It  is possi ble 
...
dbName="COUNT_SERVICEP_O">  
    ...
    <ber:attributeQuery>  
      <ber:relationQuery>  
        <ber:expression>count(*)</ber:expression>  
        <ber:relationAssoc refId="sr _5060000_5012200"/>  
        <ber:filterRelated>  
          <ogc:Filter>  
            <ogc:And>  
...
needs to be added . 
• Literal  – Specification of filtering value.  44 4 – ATTRIBUTES  
 
 
 
Implementation Guide       4.5.3 Geometry d erived attributes  
...
4.5.4 Combined  derived  attributes  
...
<ber:derivedAttribute id="at_complex" name="Complex info" dbName="COMPLEX_INFO">  
  ...
  </ber:dataType>  IMPLEMENTATION GUIDE    VERSION 8.24 45 
  ...
  Implementation Guide    <ber:attributeQuery>  
    <ber:complexQuery>  
      <ber:queryArray>  
        ...
        <ber:geometryQuery>  
          <ber:geometryFunction name="length"/>  
          <ber:expre ssion>nvl({0}, 0)</ber:expression>  
        </ber:geometryQuery>  
        <ber:relationQuery>  
          <ber:attributeArray>  
...
4.5.5 Cross sections derived attributes  
This type of attribute is intended for a cross section feature type and computes cable positions.  
Example:  Derivation of name of assigned cross section.  
  <ber:derivedAttribute id="at_e5121011" name="Cross section name" 
dbName="CS_NAME">  
    <ber:dataType>  
    ...
    <ber:attributeQuery>  
      <ber:crossSectionQuery>  
        <ber:crossSectionFunction name="c sName"/>  
      </ber:crossSectionQuery>  46 4 – ATTRIBUTES  
 
 
 
Implementation Guide           </ber:attributeQuery>  
  </ber:derivedAttribute>  
• crossSectionFunction  – Possible functions: assignedPositionsCount , csDescription  (derives 
description of assigned cross section to a route type's attribute), csName  (derives name of assigned 
cross section to a route type's attribute), freePositionsCount , maxPositionsCount . 
4.5.6 Security derived attributes  
Security derived attribute is defined for deriving a security codelist value. Definition is similar to 
ordinary attrib ute, only there is a reference to a codelist ( <ber:codeListRef refId="cl_018"… ) and its 
...
by client application. Howev er the definition is nearly same as the derived attribute definition  – you can 
use the same functions , including geometr ic (see chapter 4.5.3 ). 
...
<ber:virtualAttribute id="at_5012115" name="Material ID">  
  <ber:dataType>  
    <ber:string maxLeng th="5"/>  IMPLEMENTATION GUIDE    VERSION 8.24 47 
...
compute length. LIDS AS will check this and does not all ow to upload or start with such metadata. This 
...
the derived attribute to the new feature type. Change parents for all child fea ture types of the 
...
4.6 FeatureRef Attribute  
Special type of attributes are featureRef attributes. In fact , the visible value s represent value s from 
master feature type in 1:N  or 1:1  relation. Moreover , featureRef attributes are physically stored in 
semantic tabl e instead of association table.  
Example:  Definition of featureRef attribute.  
<ber:featureRefAttribute id="at_ 1" name=" Some name " dbName=" XY"> 
  <ber:dataType>  
    <ber:featureRef>  
      <ber:attribute refId="at_ 2"/> 
      <ber:featureRefRelationAssoc refId="r_ 1"/> 
    </ber:featureRef>  48 4 – ATTRIBUTES  
...
with the Search extension. It’s not possib le to index such attributes.  
A featureRef attribute can define more attributes from the master feature directl y. In such case, one of 
...
<ber:featureRefAttribute id="at _1" name=" Some name " dbName=" XY"> 
  <ber:dataType>  
    <ber:featureRef>  
      <ber:attribute refId="at_ 2"/> 
      <ber:attribute refId="at_ 3" default="true"/>  
      <ber:attribute refId="at_ 4"/> 
      <ber:featureRefRelationAssoc refId="r_ 1"/> 
    </ber:featureRef>  
  </ber:dataType>  
</ber:featureRefAttribute>  
...
child role. Master role does not de fine cardinality, it must be 1 in every case.  
...
Example:  Definition of featureRef RelationAssoc:  
<ber:featureRefRelationAssoc id="r_ 1" name=" Some name "> 
  <ber:masterRole refId="rt_ 1"> 
    <ber:orderBy>  
      <ber:attribute refId="at_ XYZ" direction="ascending" position="0"/>  
      <ber:attribute refId="at_ ABC" direction="ascending" position=" 1"/> 
    </ber:orderBy>  
  </ber:masterRole>  
  <ber:childRole refId="rt_ 2"/> 
</ber:featureRefRelationAssoc>  IMPLEMENTATION GUIDE    VERSION 8.24 49 
 
 
 
  Implementation Guide  4.7 Generated Attribute  
...
... 
<ber:generatedAttribute id="at_5 011005" name="Serial number" dbName="SERIAL_NUM">  
  <ber:dataType>  
    <ber:string maxLength="40"/>  
  </ber:dataType>  
  <ber:attributeQuery>  
...
• Codelist reference  (you can change a codelist data type to a differen t codelist, different codelist must 
...
multiple redefinition of one attribute.  50 4 – ATTRIBUTES  
 
 
 
Implementation Guide       Example 1:  
    ...
    <ber:redefinedSecurityDerivedAttribute refId="at_5120006_s">  
      <ber:attributeQuery>  
        <ber:relationQuery>  
          ...
          <ber:expressi on>min({0})</ber:expression>  
          <ber:relationAssoc refId="r_route_duct"/>  
        </ber:relationQuery>  
      </ber:attributeQuery>  
...
Example 2:  
Original definition o f attribute:  
...
Redefinition  1: 
      <ber:redefinedFeatureAttributeArray>  
        <ber:redefinedAttribute refId="at_5123405" nillable="false">  
          <ber:defaultValue>  IMPLEMENTATION GUIDE    VERSION 8.24 51 
 
 
 
  Implementation Guide              <ber:value/>  
          </ber:defaultValue>  
...
Redefinition 1:  
   <ber:redefinedFea tureAttributeArray>  
        <ber:redefinedAttribute refId="at_5012000_diameter">  
          ...
          </ber:defaultValue>  52 4 – ATTRIBUTES  
 
 
 
Implementation Guide               </ber:redefinedAttribute>  
      </ber:redefinedFeatureAttributeArray>  
 
Redefinition 2:  
      <ber:redefinedFeatureAttributeArray>  
        <ber:redefinedAttribute refId="at_5012000_ diameter" nillable="true"/>  
...
Redefinition:  
     <ber:redefinedFeatureAttributeArray>  
      ...
      </ber:redefinedFeatureAttributeA rray>    
  ... 
      <ber:redefinedFeatureAttributeArray>  
        <ber:redefinedDerivedAttribute refId="at_5110004">  
          ...
          </ber:attributeQuery>  IMPLEMENTATION GUIDE    VERSION 8.24 53 
 
 
 
  Implementation Guide          </ber:redefinedDerivedAttribute>  
      </ber:redefinedFeatureAttributeArray>  
4.9 Feature Info  
...
 FeatureInfo definition is inherited from the parent feature types. Child  feature ty pe can define its own 
definition.  
 Existing data needs to be refreshed after adding / modifying the definition by calling deriveFeatureInfos 
service.  54 4 – ATTRIBUTES  
...
Implementation Guide        
 ...
 Chapter 5 
 Codelists  
 IMPLEMENTATION GUIDE    VERSION 8.24 57 
 
 
 
  Implementation Guide  5. Codelists  
...
In general, code lists can have more than key and displayable column. It can have as many columns as 
needed. A codelist in general can be referenced from many feature attributes within many feature types. 
Sometimes there is a requirement to display different column as a de scription from different feature 
    ...
    <ber:codeList id="cl_105" name="Diameter" dbName="C_W_DIAMETER"  
      ...
      <ber:columnArray>  
        ...
        <ber:column id="ca_10503" name="Protection area radius" dbName="WIDTH">  
          <ber:dataType>  58 5 – CODELISTS  
 
 
 
Implementation Guide                   <ber:decimal precision="20" scale="5"/>  
          </ber:dataType>  
...
• dbNam e – The codelist database storage.  
...
images display in LIDS clients . IMPLEMENTATION GUIDE    VERSION 8.24 59 
 
 
 
  Implementation Guide   The actual codelist data (the collection of codelist items) are stored in a database. It is not defined in any 
metadata file.  
5.2 Defining Codelist Column  
...
values are true or false . If true, database is automatically expanded by additional columns contain ing 
...
Example: Codelist using outputFormat  parameter.  
    <ber:codeList id="cl_ aaa" name=" Aaa" dbName=" AAA" size="small">  
      <ber:description> Abc abc</ber:description>  
      <ber:columnArray>  
... 
        <ber:column id="ca_ aaa_1" name=" Some number " dbName=" A1 outputFormat=":F4">  
          <ber:dataType>  
            <ber:decimal precision="20" scale="5"/>  
          </ber:dataType>  
        </ber:column>  
        <ber:column id="ca_ aaa_2" name="Image" dbName="IMAGE">  
          <ber:dataType>  
            <ber:binar y contentType="image/png"/>  60 5 – CODELISTS  
...
possibility to define differe nt orders for a single codelist.  
...
<ber:model>  
   ... 
  <ber:codeListArray> <! -- A Collection of Codel ists --> 
    <ber:codeList id="cl_105" name="Diameter" dbName="C_W_DIAMETER"  
                  size="small">  
      <ber:description>Diameter</ber:description>  
      <ber:columnArray>  IMPLEMENTATION GUIDE    VERSION 8.24 61 
  ...
  Implementation Guide   
        ...
        <ber:column id=" ca_10501" name="Identifier"  
          ...
          <ber:description>Identifier - Primary key</ber:description>  
          <ber:dataType>  
            <ber:decimal precisio n="5" scale="0"/>  
          </ber:dataType>  
        </ber:column>  
        ...
        <ber:column id="ca_10503" name="Code" dbName="CODE"   
          ...
          <!-- Sort ascending according th e code -->           
          <ber:orderBy direction="ascending" position="0" />  
        </ber:column>  
      </ber:columnArray>  
    </ber:codeList>  
...
the priority of ordering).  62 5 – CODELISTS  
 
 
 
Implementation Guide       5.5 Hierarchical Codelists  
...
The hierarc hical XML attribute of a codelist indicates that this codelist is a hierarchical one. In the 
...
<ber:codeList id="cl_008"  
              ...
              hierarchical="true" > 
  <ber:description>Ser vice organization unit -  
                   hierarchical</ber:description>  
  <ber:columnArray>  
    <ber:column id="ca_00801" name="Identifier"  
                dbName="C_G_SERVICE_OU"  
                nillable="false" primaryKey="true">  
      <ber:descrip tion>Identifier - Primary key</ber:description>  
      <ber:dataType>  
        <ber:decimal precision="5" scale="0"/>  
      </ber:dataType>  
    </ber:column>  
    ...
    <ber:column id="ca_00804" name="Superkey"  
               ...
               superKey="true" > 
      <ber:description>Identifier - Super key</ber:description>  IMPLEMENTATION GUIDE    VERSION 8.24 63 
 
 
 
  Implementation Guide        <ber:dataType>  
        <ber:decimal pr ecision="5" scale="0"/>  
      </ber:dataType>  
    </ber:column>  
  </ber:columnArray>  
</ber:codeList>  
5.6 Security Codelists  
In fact, security codelists are quite similar to a standard codelists but have a special meaning in the 
project. The security codelist d efines row level access rights to features.  
...
secRefNullValue  defin ed. They describe the database references to the codelist.  
 For performance reasons the feature attributes referring security codelists are stored completely 
differently than the feature attributes referring normal codelists. As the references to normal cod elists 
...
<ber:securityCodeList id="cl_008"  
  ...
  <ber:description>Servic e organization unit – hierarchical,  
...
Implementation Guide           <ber:column id="ca_00802" name="Unit ID" dbName="UNIT_ID"  
         nillable="fa lse"> 
      <ber:dataType>  
        <ber:string maxLength="40"/>  
      </ber:dataType>  
...
A feature type can refer the security codelist in two possible ways. The first way is a sta ndard way: the 
...
<ber:securityA ttribute  id="at_5005312" name="Service ID">  
  <ber:dataType>  
...
attribute in a security codelist definition.  IMPLEMENTATION GUIDE    VERSION 8.24 65 
 
 
 
  Implementation Guide  5.7 Codelist Binding  
Records from two or more codelists can be bounded. That means that only some combinations of records 
from the codelists are allowed; they are defined in the codelist binding. A featur e type can define that 
...
5.7.1 Codelist binding definition  
...
 Ordered codelist binding is a speci al kind of binding which defines order of individual refererred 
codelists. This flag influences the client behaviour during editing the bounded attributes. The resulting 
state – the allowed combinations are the same in case of order and not ordered definit ion. 
...
Implementation Guide               <!-- A description column --> 
        <ber:column id="ca_10502" name="Description"  
                    dbName="DESCRIPTION"  
                    nillable="false">  
          <ber:dataType>  
    ...
    <!-- A Material Codelist --> 
    <ber:codeList id="cl_102" name="Material" dbName="C_W_MATERIAL"  
                  size="small">  
      <ber:description>Materi al</ber:description>  
      <ber:columnArray>  
        <ber:column id="ca_10201" name="Identifier"  
                    dbName="C_W_MATERIAL"  
                    nillable="false" primaryKey="true">  
          <ber:description>Identifier - Primary key</ber:des cription>  
          <ber:dataType>  
            <ber:decimal precision="5" scale="0"/>  
          </ber:dataType>  
        </ber:column>  
 
        <ber:column id="ca_10202" name="Description"  
                    dbName="DESCRIPTION">  
          <ber:dataType>  
            <ber:string maxLength="80"/>  
          </ber:dataType>  
  ...
  <ber:codeListBindingArray >  
     <!-- A Collection of codelist bindings --> 
     <ber:codeList Binding id="bc_1" dbName="STD_MATERIAL_DIAMETER"  
                          name="Material - Diameter Binding"  ordered="false" > 
       <ber:description>Material – Diameter Codelist  
                        Binding</ber:description>  
        IMPLEMENTATION GUIDE    VERSION 8.24 67 
 
 
 
  Implementation Guide         <ber:bounde dItemArray>  
...
Example: An attr ibute binding between the Material and Diameter attribute for the Pipeline feature 
type.  
<ber:featureTypeArray>  
  ... 
  <ber:featureType  
     ...
     name="Pipeline" parentId="ft_5010000"  
           abstract="true">  
    <ber:description>Wat er pipeline</ber:description>  
     68 5 – CODELISTS  
 
 
 
Implementation Guide           <ber:featureAttributeArray> <! -- Array of Feature Attributes --> 
     <ber:attribute id="at_5010004" name="Note" dbName="NOTE">  
        <ber:description>Notes for the record</ber:description>         
        <ber:dataType>  <! -- The Data Type Definition --> 
          <ber:string maxLength="80"/> <! -- The string value --> 
      ...
      <ber:attribute id="at_5012001" name="Diameter"  
             ...
             nillable="false">         
        <ber:dataType> <! -- Reference to a code list --> 
          <ber:codeListRef refId="cl_105" displa yColumn="ca_10502"/>  
        </ber:dataType>  
      </ber:attribute>  
 ...
 IMPLEMENTATION GUIDE    VERSION 8.24 69 
 
 
 
  Implementation Guide          <!-- The Diameter attribute --> 
...
• Security codelist s can be defined as checkboxed as well.  
• Checkboxed codelists can be used as data types of codelists columns as well.  70 5 – CODELISTS  
 
 
 
Implementation Guide       Example: Checkboxed codelist.  
<ber:codeList id="cl_006" name="Status" dbName="C_G_STATUS" size="small">  
 ...
 Chapter 6  
 Interface s 
 IMPLEMENTATION GUIDE    VERSION 8.24 73 
 
 
  Implementatio n Guide  6. Interface s 
Defines new ” view ” at the data – it can bring together heterogeneous data and make them look 
homogenous. Interface  definition includes attributes and forms defi nitions . 
  ...
  <ber:interfaceArray>  
    <ber:interface id="if_1" name=" Interface 1 "> 
      ...
      <ber:interfaceAttributeArray>  
        <ber:interfaceAttribute id="ia_1_0 1" name=" Name" dbName=" NAME"> 
          <ber:description> String attribute </ber:description>  
          <ber:dataType>  
            <ber:string maxLength="80"/>   
          </ber:dataType>  
        </ber:interfaceAttribute>  
        <ber:interfaceAttribute id="ia_1_0 2" name=" Length" dbName=" LENGTH"> 
          <ber:description> Decimal attribute </ber:description>  
          <ber:dataType>  
            <ber:decimal precision=" 7" scale=" 2"/> 
          </ber:dataType>  
        </ber:interfaceAttribute>  
        <ber:interfaceAttribute id="ia_1_ 03" name="Type" dbName="TYPE">  
          <ber:description> Codelist attribute </ber:description>  
          <ber:dataType>  
            <ber:codeListRef refId="cl_ 1" displayColumn="ca_ 1_01"/> 
          </ber:dataType>  
        </ber:interfaceAttribute>  
        <ber:interfaceAttribute id="ia_1_ 04" name=" Master ID" dbName=" MASTER_ID"> 
          <ber:description> FeatureRef attribute </ber:description>  
          <ber:dataType>  
            <ber:featureRef>  
              <ber:attribute refId="at_ r_002"/> 
              <ber:featureRefRelationAssoc refId="r_ a_b"/> 
            </ber:featureRef>  
          </ber:dataType>  
        </ber:interfaceAttribute>  
      </ber:interfaceAttributeArray>  
      <ber:formArray>  
        <ber:form id="frm_if_101" name=" Interface 1 "> 
          <ber:description>  Heterogeneous list 1 </ber:description>  
          <ber:fieldGroup>  74 6 – INTERFACE S 
 
 
 
Implementation Guide                   <ber:field refId="ia_1_01"/>  
            <ber:field refId="ia_1_02"/>  
            <ber:field refId="ia_1_03"/>  
            <ber:field refId="ia_1_0 4"/> 
          </ber:fieldGroup>  
        </ber:form>  
      </ber:formArray>  
    </ber:interface>  
...
6.1 Defining Interface Attributes  
...
and precision (:f2 or :F0).  
Inner elements  
• description  – Detailed descriptio n of an interface attribute.  
• dataType  – Data type of an interface attribute or reference to an association 
(<ber:featureRefRelationAssoc) and its attribute (<ber:attribute).  
6.2 Implementing Interface  
...
• refId  – Interface definition reference . IMPLEMENTATION GUIDE    VERSION 8.24 75 
 
 
  Implementatio n Guide  • interfaceAttributeDef  
...
Implementation Guide        
 ...
 Chapter 7 
 Forms  
 IMPLEMENTATION GUIDE    VERSION 8.24 79 
 
 
 
  Implementation Guide  7. Forms  
...
<ber:featureTypeArray>  
  ... 
  <ber:featureType id="ft_ pipe" name="Pipeline" parentId="ft_ parent"> 
  ... 
    <ber:featureAttributeArray> <! -- Array of Feature Attributes --> 
      <ber:attribute id="at_ pipe_1" name="Diameter" dbName="C_W_DIAMETER"  
             nillable="false">         
        <ber:dataType> <! -- Reference to a code list --> 
          <ber:codeListRef refId="cl_105" displayColumn="ca_10502"/>  
        </ber:dataType>  
      </ber:attribute>  
      <ber:attribute id="at_ pipe_2" name="Length [m]" dbName="LENGTH">  
        <ber:description>The length of a pipeline</ber:description>         
        <ber:dataType>  <!-- Decimal number --> 
          <ber:decimal precision="10" scale="2"/>  
        </ber:dataType>  
      </ber:attribute>  
      <ber:attribute id="at_ pipe_3" name="Material" dbName="C_W_MATERIAL"  
             nillable="false">  
        <ber:dataType> <! -- Reference to a code list --> 
          <ber:codeListRef refId="cl_102" displayColumn="ca_ 10202"/>  
        </ber:dataType>  
      </ber:attribute>  
      <ber:attribute id="at_ pipe_4" name="Note" dbName="NOTE"  nillable="true">  
        <ber:description>Notes for the record</ber:description>         
        <ber:dataType>   <!-- String value --> 
          <ber:string maxLength="80"/>  
        </ber:dataType>  
      </ber:attribute>  
      <ber:featureRefAttribute id="at _pipe_5" name=" Master feature " dbName=" FREF"> 
        <ber:dataType>  
          <ber:featureRef>  
            <ber:attribute refId="at_ master_1 "/> 
            <ber:attribute refId="at_ master_2 " default="true"/>  80 7 – FORMS  
 
 
 
Implementation Guide                   <ber:attribute refId="at_ master_3 "/> 
            <ber:featureRefRelationAssoc refId="r_ 1"/> 
          </ber:featureRef>  
        </ber:dataType>  
      </ber:featureRefAt tribute>  
    </ber:featureAttributeArray>  
  ... 
    <ber:formArray> <! -- Array of Forms --> 
      <ber:form id="fm_ pipe_1" name=" Pipeline – complex"> 
        <ber:fieldGroup>           
          <ber:field id="f_1" refId="at_ pipe_4" readOnly="false"  
                     label="Description" length="80"/>  
          <ber:field refId="at_ pipe_2" readOnly="false" length="80"/>  
        </ber:fieldGroup>  
        <ber:fieldGroup>  
          <ber:separator/>  
          <ber:field refId="at_ pipe_1" readOnly="fals e"  
                     label="Pipe diameter" length="80"/>  
          <ber:field refId="at_ pipe_3" readOnly=" true" length="80"/>  
        </ber:fieldGroup>  
        <ber:fieldGroup>  
          ...
          <ber:field attributePath="at_pipe_5/at_master_1" label="Master name"  
                      length="90"/>  
          <ber:field attributePath="at_pipe_5/at_mast er_3" label="Master type"/>  
        ...
        <ber:fieldGroup>  
          ...
          <ber:field refId="updatedBy" label="Updated by" l ength="90"/>  
          ...
          <ber:fiel d refId="longTransactionId" length="40"/>  
        </ber:fieldGroup>  
      </ber:form>  IMPLEMENTATION GUIDE    VERSION 8.24 81 
 
 
 
  Implementation Guide        <ber:form id=" fm_pipe_2 " name=" Pipeline – reduced, read-only"> 
        <ber:fieldGroup>  
          <ber:field refId="at_ pipe_1" readOnly=" true" length=" 45"/> 
          <ber:field refId="at_ pipe_3" readOnly=" true" length=" 120"/> 
        </ber:fieldGroup>  
      </ber:form>  
      <ber:assignedForms>  
        <ber:assignedForm usage=" default" refId="fm_pipe_ 1"/> 
        <ber:assignedForm usage=" insertSemantics " refId="fm_pipe_ 1"/> 
        <ber:assignedForm usage=" insertGraphics " refId="fm_pipe_ 1"/> 
        <ber:assignedForm usage=" updateSemantics " refId="fm_pipe_ 1"/> 
...
o collapsible  – allowed  values are: expanded|collapsed|false . Default is false . 82 7 – FORMS  
 
 
 
Implementation Guide        
Feature type can define any number of forms. Forms  can be assigned to the specific actions using  
assignedForms  section  (list ca n be extended according to the application  needs) : 
...
• locate  – This form is used when locating featur e in graphic view.  
...
• mobileFull – This form is used for displaying of the single feature in LIDS Mobile.  
 ...
 Chapter 8 
 Feature Graphics  
 IMPLEMENTATION GUIDE    VERSION 8.24 85 
 
 
 
  Implementation Guide  8. Feature Graphics  
...
specify its master graphics. It can be either primiti ve or collection graphics.  
...
every feature of this fe ature type must have its master graphics created.  
...
Examp le: Defining primitive master graphics for Main Pipeline segment.  
<ber:model>  
  <ber:featureTypeArray>  
    ... 
 ...
 86 8 – FEATURE GRAPHICS  
 
 
 
Implementation Guide       Collection Master Graphics  
Some special features have collection master graphics, i.e. t heir master graphics is a collection of 
...
active manipulations with grap hic collections. It is a job for system extensions. Every collection graphic  
        ...
        <ber:collectionGraphicType>  
          <ber:graphicTypeComponentArray>  
            ...
            <ber:graphicTypeComponent id="gtc_02" name="Extension Line"  
                                      roleId="extLine">  IMPLEMENTATION GUIDE    VERSION 8.24 87 
 
 
 
  Implementation Guide                <ber:curveGraphicType>  
                <ber:symbology>  
                ... 
                </ber:symbology>  
              </ber:curveGraphicType>  
...
are defined in special part of metadata and refer red from feature type definitions.  88 8 – FEATURE GRAPHICS  
 
 
 
Implementation Guide       Example: Defining graphic tags – Displayable Attribute.  
<ber:model>  
  ... 
  <ber:system id="0">  
    ... 
  ...
  <ber:graphicTypeArray>  
    <ber:graphicType id="gtda_5012102" n ame="W_DA_5012102">  
      <ber:class refId="DA"/>  
      <ber:collectionGraphicType>  
        <ber:symbology>  
          ... 
  ...
  ... 
  <ber:featureTypeArray>  
    ... IMPLEMENTATION GUIDE    VERSION 8.24 89 
 
 
 
  Implementation Guide      <ber:featureType id="ft_5012100"  name="Main pipeline segment"  
...
• graphicTypeArray  – Defines graphic tag types. In this case one graphi c tag type is defined. It is a 
...
types which can be a ttached to feature type.  
8.2.1 Border line  
...
attribute, or of axis geometry automatically implies a correspo nding change of borders. If more lines 
intersect, the system provides modification of their borders at the intersection so that the borders are 
connected.  
 
Possi ble variants of the definition:  
...
• Axis with both borders and closing lines.  
 90 8 – FEATURE GRAPHICS  
 
 
 
Implementation Guide       Either a  graphic type containing borders and / or closing lines components must be defined for the line 
feature type as its graphic tag  or separate feature type with relation association to the axis  can be defined 
(in model.xml ). Further, the following mus t be specified in the option.xml  file:  
...
• Feature type and re lationAssoc type. Optionally, default values can be specified  
...
the bord ers are not generated for such a feature. If borders already existed for such a feature, they 
...
tag</ber:description>  
      </ber:graphicTypeClass>  IMPLEMENTATION GUIDE    VERSION 8.24 91 
 
 
 
  Implementation Guide      </ber:graphicTypeClassArray>  
  </ber:system>  
  ... 
...
roleId="closingLine">  
 ...
 92 8 – FEATURE GRAPHICS  
 
 
 
Implementation Guide       option.xml:  
   <ber:lineBorder id="bod_3" name="Route">  
      ...
      <ber:outputLineBorder>  
        <ber:diameter>  
          <ogc:PropertyName>at_e5121006/ca_31903</o gc:PropertyName>  
        </ber:diameter>  
    ...
    <ber:lineBorder id=" line_border_2 " name=" Just on right side"> 
      ...
      </ber:inputAxis>  IMPLEMENTATION GUIDE    VERSION 8.24 93 
 
 
 
  Implementation Guide        <ber:outputLineBorder>  
...
basis of a rule defined in metadata. The wi dth of the area can be  controlled by values of selected attribute. 
...
Optionally, following conditions c an be defined:  
• excludeFeatures  – Condition  for excluding features  from the processing  can be defined. If valid, 
the buffer zone is not generated for such a feature.  If buffer zone already existed for such a feature, 
it’s left untouched. Also modification o f the source geometry doesn’t cause any update on the 
generated border  
...
• A constant  – the user can’t influence the width  
 94 8 – FEATURE GRAPHICS  
 
 
 
Implementation Guide       Possible definition of the buffer zone’s shape:  
• joinStyle  – Style  of the outer boundary  at the internal vertices of the source geometry . Possible 
values : 
• mitre  - Sharp line, represented by one vertex at each source vertex  
• round – Rounded line, represented by more vertexes at each source vertex  
...
source line  
• round – Line with more int ernal vertexes created in the distance of buffer zone’s width from 
the source line  
...
polygon">  
      ...
      <ber:inputAxis>  
        ...
        <ber:excludeFeatures>  
          <ogc:Filter> <! -- Material in 'Unknown', 'Wood' --> IMPLEMENTATION GUIDE    VERSION 8.24 95 
 
 
 
  Implementation Guide              <ber:PropertyIsIn>  
              <ogc:PropertyName>ft_5012300/at_5012003</ogc:PropertyName>  
              <ogc:Literal> -1</ogc:Literal>  
        ...
        <ber:deleteBorderForFeatures>  
          <ogc:Filter> <! -- Supplying zone = 'Low pressure' --> 
            <ogc:PropertyIsEqualTo>  
              <ogc:PropertyName>ft _5012100/at_5010001</ogc:PropertyName>  
              <ogc:Literal>1</ogc:Literal>  
            </ogc:PropertyIsEqualTo>  
          </ogc:Filter>  
        </ber:deleteBorderForFeatures>  
              ...
              <ber:symbology>  
                <ber:polygonSymbolizer>  
                  <ber:fill>  
                    ...
                    <se:SvgParameter name="fill -opacity">0.3</se:SvgParameter>  96 8 – FEATURE GRAPHICS  
 
 
 
Implementation Guide                           <ber:fillStyle>solid</ber:fillStyle>  
                  </ber:fill>  
                  <se:Stroke>  
...
</ber:model>  IMPLEMENTATION GUIDE    VERSION 8.24 97 
 
 
 
  Implementation Guide  8.2.3 Other classes  
Example:  Defining graphic tags – Simple Dimension . In this case, the dimension line is the master 
geometry which is c reated by standard functions. Symbols at both ends and the text are graphic tags . 
<ber:model>  
  ... 
  <ber:system id="0">  
    ... 
    <ber:graphicTypeClassArray>  
      <ber:graphicTypeClass id="SimpleDimension" name="Simple dimension">  
        <ber:descrip tion>Dimensions for LIDS Browser, graphic tag</ber:description>  
      </ber:graphicTypeClass>  
    </ber:graphicTypeClassArray>  
  </ber:system>  
  ... 
...
roleId="symbEnd">  
            <ber:pointGraphicType>  
                <ber:symbology>  98 8 – FEATURE GRAPHICS  
 
 
 
Implementation Guide                         <ber:pointSymbolizer>  
                    <se:Graphic>  
                      <se:ExternalGraphic>  
                        <se:OnlineResource 
xlink:href="resource.xml#ber_LIDSdemo/76/left -center"/>  
...
roleId="dimText">  
              <ber:textGraphicType>  
                <ber:symbology>  
                  <ber:textSymbolizer>  
                    <se:Font>  
                      <se:SvgParam eter name="font -family">Courier</se:SvgParameter>  
...
<ber:model>  
  ... 
  <ber:system id="0">  
    ... IMPLEMENTATION GUIDE    VERSION 8.24 99 
...
roleId="auxiliaryGraphics">  100 8 – FEATURE GRAPHICS  
...
roleId="labelLine">  
            <ber:curveGraphicType>  
              <ber:symbology>  
                <ber:lineSymbolizer>  
                  <se:Stroke>  
                    <se:SvgParameter name="stroke -width">1</se:SvgParam eter> 
                  </se:Stroke>  
                </ber:lineSymbolizer>  
              </ber:symbology>  
            </ber:curveGraphicType>  IMPLEMENTATION GUIDE    VERSION 8.24 101 
 
 
 
  Implementation Guide            </ber:graphicTypeComponent>  
        </ber:graphicTypeComponentArray>  
      </ber:collectionGraphicType>  
    </ber:graphicType>  
...
roleId="labelSymbol">  
            <ber:pointGraphicType>  
              <ber:symbology>  102 8 – FEATURE GRAPHICS  
...
xlink:href="resource.xml#ber_LIDSdemo/51"/>  
                  ...
                  <ber:symbolHeight>1</ber:symbo lHeight>  
                </ber:pointSymbolizer>  
              </ber:symbology>  
            </ber:pointGraphicType>  
          </ber:graphicTypeComponent>  
...
roleId="text">  
            <ber:textGraphicType>  
              ...
              <ber:defaultTextPro perties>  
                <ber:anchorPoint>center -center</ber:anchorPoint>  
              </ber:defaultTextProperties>  
            </ber:textGraphicType>  
          </ber:graphicTypeComponent>  
          <ber:graphicTypeComponent id="gtcsr_01_4" name="Directio n symbol left" 
...
xlink:href="resource.xml#ber_LIDSdemo/53/right -center"/>  
                ...
                </ber:pointSymbolize r> 
              </ber:symbology>  
            </ber:pointGraphicType>  
          </ber:graphicTypeComponent>  
          <ber:graphicTypeComponent id="gtcsr_01_5" name="Direction symbol right" 
roleId="rightDirection">  IMPLEMENTATION GUIDE    VERSION 8.24 103 
 
 
 
  Implementation Guide              <ber:pointGraphicType>  
              <ber:symbology>  
                <ber:pointSymbolizer>  
                  <se:Graphic>  
                    <se:ExternalGraphic>  
...
roleId="labelLine">  104 8 – FEATURE GRAPHICS  
...
xlink:href="resource.xml#ber_LIDSdemo/51"/>  
          ...
          </ber:gra phicTypeComponent>  
          <ber:graphicTypeComponent id="gtdr_5070001_3" name="Detail Number" 
roleId="text">  
            <ber:textGraphicType>  
              <ber:symbology>  
                <ber:textSymbolizer>  
                  <ber:fontHeight>1</ber:fon tHeight>  
                </ber:textSymbolizer>  
              </ber:symbology>  
              <ber:defaultTextProperties>  
                <ber:anchorPoint>center -center</ber:anchorPoint>  
              </ber:defaultTextProperties>  
            </ber:textGraphi cType> 
          </ber:graphicTypeComponent>  
        </ber:graphicTypeComponentArray>  
      </ber:collectionGraphicType>  
    </ber:graphicType>  IMPLEMENTATION GUIDE    VERSION 8.24 105 
 
 
 
  Implementation Guide  Example: Defining graphic tags – Detail reference in map frame.  
<ber:model>  
  ... 
  <ber:system id="0">  
    ... 
...
roleId="detailFrame">  
            <ber:surfaceGraphicType>  
              ...
              </ber:symbology>  106 8 – FEATURE GRAPHICS  
 
 
 
Implementation Guide                   </ber:surfaceGraphicType>  
          </ber:graphicTypeComponent>  
        </ber:graphicTypeComponentArray>  
      </ber:collectionGraphicType>  
    </ber:graphicType>  
  ...
  <ber:system id="0">  
    ...
    <ber:graphicTypeClassArray>  
      <ber:graphicTypeClass id="GhostSymbol" name="Ghost symbol">  
        <ber:description>Graphic tag type consisting of one or more poin t symbols 
...
xlink:href="resource.xml#ber_LIDSdemo/64/ center-center"/>  
          ...
          <ber:graphicTypeComponent id="ghostsymb_01_02" name="Rectangle">  
            <ber:pointGraphicType>  IMPLEMENTATION GUIDE    VERSION 8.24 107 
 
 
 
  Implementation Guide                <ber:symbology>  
                <ber:pointSymbolizer>  
                  <se:Graphic>  
                    <se:ExternalGraphic>  
                      <se:OnlineResource 
          ...
          <ber:graphicTypeComponent id="freegt_02_01" name="Arrow">  
            <ber:pointGraphicType>  
              <ber:symbology>  
                <ber:pointSymbolizer>  
                  <se:Graphic>  
                    <se:ExternalGraphic>  108 8 – FEATURE GRAPHICS  
 
 
 
Implementation Guide                             <se:OnlineResource 
xlink:href="resource.xml#ber_LIDSdemo/45"/>  
                      <se:Format/>  
                    </se:ExternalGraphic>  
                  </se:Graphic>  
                  <ber:rgbColor>#CC 0000</ber:rgbColor>  
                </ber:pointSymbolizer>  
              </ber:symbology>  
            </ber:pointGraphicType>  
          </ber:graphicTypeComponent>  
...
xlink:href="resource.xml#ber_LIDSdemo/35"/>  
          ...
          <ber:graphicTypeComponent id="freegt_02_04" name="Point empty">  IMPLEMENTATION GUIDE    VERSION 8.24 109 
...
xlink:href="resource.xml#ber_LIDSdemo/51"/>  
          ...
          <ber:graphicTypeComponent id="freegt_02_05" nam e="Polygon">  
            <ber:surfaceGraphicType>  
              <ber:symbology>  
                <ber:polygonSymbolizer>  
                  <ber:fill>  
                    <se:SvgParameter name="fill">#808080</se:SvgParameter>  
                    <ber:fillSty le>solid</ber:fillStyle>  
                  </ber:fill>  
                  <se:Stroke>  
          ...
          <ber:graphicTypeComponent id="freegt_02_06" name="Line">  
            <ber:curveGraphicType>  
              <ber:symbology>  
                <ber:lineSymbolizer>  
                  <se:Stroke>  
                    <se:SvgParameter name="stroke">#CC0000</se:SvgPara meter> 
                    <se:SvgParameter name="stroke -width">2</se:SvgParameter>  
                  </se:Stroke>  
                </ber:lineSymbolizer>  
              </ber:symbology>  
            </ber:curveGraphicType>  
          </ber:graphicTypeComponent>  110 8 – FEATURE GRAPHICS  
 
 
 
Implementation Guide                 <ber:graphicTypeComponent id="freegt_02_07" name="Text">  
            <ber:textGraphicType>  
              <ber:symbology>  
                <ber:textSymbolizer>  
                  <se:Fill>  
...
roleId="directionSymbol">  
            <ber:pointGraphicType>  
              <ber:symbolog y> 
                <ber:pointSymbolizer>  
                  <se:Graphic>  IMPLEMENTATION GUIDE    VERSION 8.24 111 
 
 
 
  Implementation Guide                      <se:ExternalGraphic>  
                      <se:OnlineResource 
...
roleId="closingLine">  
            <ber:curveGraphicType/>  
          </ber:graphicTypeComponent>  112 8 – FEATURE GRAPHICS  
 
 
 
Implementation Guide               </ber:graphicTypeComponentArray>  
      </ber:collectionGraphicType>  
    </ber:graphicType>  
...
<ber:model>  
  ...
  <ber:system id="0">  
    ... 
    <ber:graphicTypeClassArray>  
      <ber:graphicTypeClass id="BorderPolygonReduction" name="Re duction - polygon">  
        <ber:description>This graphic type is used for reduction consisting of 
polygon.</ber:description>  
  ...
  ... 
   <ber:graphicType id="reductionP_01" name=" Reduction - polygon 1">  
    ...
    </ber:graphicType>  IMPLEMENTATION GUIDE    VERSION 8.24 113 
 
 
 
  Implementation Guide  8.2.4 Mapping graphic tag types for change feature type function  
Function change featur e type enables preserving of defined graphic tags of the source feature after 
converting to target feature type. GraphicTagType definition is unique for every feature type and 
different feature types can’t share the same graphicTagType definition. That’s w hy some additional 
mapping is necessary. The mapping is d efined  in graphicTagTypeMappingArray  in option.xml . 
...
Example: Defining graphic tags  mapping in option.xml . 
  <ber:graphicTagTypeMappingArray>  
    ...
    <ber:graphicTagTypeMapping id=" m_3" name="Direction symbols">  
      <ber:graphicTagT ype xlink:href="model.xml#ft_ A_dirsymb_01"/>  
      <ber:graphicTagType xlink:href="model.xml#ft_ A_dirsymb_02"/>  
      <ber:graphicTagType xlink:href="model.xml#ft_ A_dirsymb_03"/>  
      <ber:graphicTagType xlink:href="model.xml#ft_ B_dirsymb_01"/>  114 8 – FEATURE GRAPHICS  
 
 
 
Implementation Guide             <ber:graphicTagType xlink:href="model.xml#ft_ C_dirsymb_01"/>  
    </ber:graphicTagTypeMapping>  
    <ber:graphicTagTypeMapping id="m_ 4" name="Free  graphic tags "> 
      <ber:graphicTagType xlink:href="model.xml#ft_A_freegt_01"/>  
      <ber:graphicTagTyp e xlink:href="model.xml#ft_ A_freegt_02"/>  
      <ber:graphicTagType xlink:href="model.xml#ft_B_freegt_01"/>  
      <ber:graphicTagType xlink:href="model.xml#ft_ C_freegt_0 1"/> 
...
definit ions can usually be reconfigured by the user interactively depending on the particular client 
application functionality.  IMPLEMENTATION GUIDE    VERSION 8.24 115 
 
 
 
  Implementation Guide  8.3.1 Feature types  
...
• layer defined as a selection of individual features – order of feature ty pes is set alphabetically by the 
the feature type name  
8.3.2 Graphic tag types  
Individual graphic tag types are ordered inside a feature type by the optional drawPriority  attribute in 
model.xml:  
• smalle r value moves the graphic tag type up in the order  
• bigger  value moves the graphic tag type down  
• value 0 means the order of master graphics  
8.3.3 Graphic tag and collection com ponents  
...
Implementation Guide        
 ...
 Chapter 9 
 Feature Symbology  
 IMPLEMENTATION GUIDE    VERSION 8.24 119 
 
 
 
  Implementation Guide  9. Feature Symbology  
...
type based. It can be either static (the s ame for all instances of the feature type in any scale) or dynamic 
(varies according to the current scale and/or feature attribute values). Every feature type can define one 
default static and one default dynamic symbology. While the static symbology is th e part of the feature 
...
properties (partial symboli zers) and the system itself fills the missing properties from some super 
symbology definition in any part of metadata. The super symbology varies for various symbology 
definitions.  
9.1 Static Symbology  
...
9.1.1 Curve symbology  
Example:  Curve symbology definition.  
<ber:featureType id="ft_5012100" name="Main pipeline segment"  
                 parentId="ft_5012000" abstract="false">  
  <ber:description>Main pipeline segment</ber:d escription>  
  <ber:masterGraphics nillable="true">  
    <ber:curveGraphicType>  
      <ber:symbology>  
        <ber:lineSymbolizer>  
          <se:Stroke>  
            ...
            <se:SvgParameter name="stroke">#1C32FF</se:Sv gParameter>  
            <se:SvgParameter name="stroke -opacity">1</se:SvgParameter>  
            <se:SvgParameter name="stroke -width">2</se:SvgParameter>  120 9 – FEATURE SYMBOLOGY  
...
stroke  – Specifies the solid color that wil l be used for a stroke. The color value is RGB -encoded 
...
stroke -width  – Specifies the absolute width (t hickness) of a stroke in pixels encoded as a float.  
 The line width used for displaying can be different than the one used for printing. It’s possible to map a 
display width to the printing width (specified in device independent point s DIP - 1/96 inch). The  
  ...
  <ber:plotting>  
    ...
    <ber:defaultNortharrow>  
      <ber:pointSymbolizer>  
        ...
        <ber:rgbColor>#C80 000</ber:rgbColor>  
        <ber:symbolHeight>36</ber:symbolHeight>  
      </ber:pointSymbolizer>  
    </ber:defaultNortharrow>  
    <!-- Line weights mapping --> 
    <ber:lineWeights>  
      <ber:lineWeight display="0" print="0.025"/>  IMPLEMENTATION GUIDE    VERSION 8.24 121 
 
 
 
  Implementation Guide        <ber:lineWeight dis play="1" print="0.125"/>  
      ...
      <ber:lineWeight display="6" pr int="1"/>  
    </ber:lineWeights>  
  </ber:plotting>  
 User line style 
...
The list of supporte d formats differs for different client applications:  
...
 Definition in XML format must be prepare d for purposes of map outputs for any type of client 
application.  
...
basic information about line style in resource.x ml. Detailed d efinition of each style listed in resource.xml  
...
need any xml -definition (style ls_0 in past).  122 9 – FEATURE SYMBOLOGY  
 
 
 
Implementation Guide       User line style logically consists of one or more line components optionally shifted from a curve's axis 
...
• perpendicular distance of shifting fr om a curve's axis - component will be drawn in this distance  
...
• optionally symbol to be drawn  at the b eginning of curve  
...
• absolute – rotation t owards global coordinate system  
...
• text 
• Drawing mode  
• continuous – the parts are drawn continuously along the w hole curve; in inner vertexes a dash -
stroke is eventually “wrapped”  
...
segment length must be gr eater than a sum of lengths of dash -stroke parts  
...
• center in se gment mode – a dash -stroke is drawn only if at once segment length is grater or 
equal to a sum of lengths of dash -stroke parts and  is grater or equal to specified minimal length  IMPLEMENTATION GUIDE    VERSION 8.24 123 
 
 
 
  Implementation Guide  • Way how to solve situation when there is not enough space for a dash -stroke p art or when some of 
...
• definition of used symbol  
• symbol size - units are taken over from the parent li ne component  
...
uom="http://www.opengeospatial.org/se/units/pixel">  
      <ber:stroke -dasharray>14 -5 14 -5 20[text_1] -5 14 -5 14 -5</ber:stroke -
dasharray>  
...
Example: Line style with symbol component  
  ...
  <ber:lineStyleArray>  
  ... 
IMPLEMENTATION GUIDE    VERSION 8.24 125 
 
 
 
  Implementation Guide      <ber:lineStyle id="ls_VLEK">  
      <ber:components>  
...
uom="http://www.opengeospatial.org/se/units/pixel">  
 ...
 126 9 – FEATURE SYMBOLOGY  
 
 
 
Implementation Guide       Example:  Complex line style  
    ...
    <ber:lineStyle id="ls_ZELEZ_DRAHA">  
      <ber:components>  
        ...
        <ber:component refId="comp_zeldraha">  
          <ber:perpendicularOffset>0</ber:perpend icularOffset>  
        </ber:component>  
...
IMPLEMENTATION GUIDE    VERSION 8.24 127 
 
 
 
  Implementation Guide              <se:Size>10</se:Size>  
            <se:Displacement>  
              <se:DisplacementX>0</se:Displaceme ntX> 
              <se:DisplacementY>0</se:DisplacementY>  
            </se:Displacement>  
          </se:Graphic>  
          <ber:rotation type="relative">0</ber:rotation>  
...
uom="http://www.opengeospatial.org/se/units/pixel">  
      ...
      <ber:segmentMode>continue</be r:segmentMode>  
      ...
      <ber:strokeArray>  
        <ber:pointStroke id="symbol_1">  
          <ber:pointSymbolizer>  
            <se:Graphic>  
              <se:ExternalGraphic>  
                <se:OnlineRe source xlink:href="resource.xml#ber_LIDSuap1/98"/>  
                <se:Format/>  
              </se:ExternalGraphic>  
              <se:Size>20</se:Size>  
              <se:Displacement>  
...
uom="http://www.opengeospatial.org/se/units/pixel">  
      ...
      <ber:segmentMode>continue</ber:segmentMode>  
      <ber:outOfStrokeGlue>solid</b er:outOfStrokeGlue>  128 9 – FEATURE SYMBOLOGY  
 
 
 
Implementation Guide           </ber:component>  
...
<ber:featureType id="ft_5060000" name="Service organization unit"  
  ...
  <ber:masterGraphics nillable="true">  
    ...
    </ber:surfaceGraphicType>  IMPLEMENTATION GUIDE    VERSION 8.24 129 
 
 
 
  Implementation Guide    </ber:masterGraphics>  
  ... 
</ber:featureType>  
...
• polygonSymbolizer  – SE based symbology defin ition for surface geometries. Only the above set of 
subelements is supported in LIDS.  
• fill – Specifies how the area of the surface will be filled. The meanings of its SvgParameters are 
similar to the Stroke  parameters discussed in conjunction with curve sy mbology above. Here, the 
...
definition but it can be useful in project defaul t symbolizer definition ( presentation.xml ). 
...
is discussed in curve symbology definition above.  
 User line style  
To display surface border, it is possible to u se user line styles.  
For more details see chap ter 9.1.1.1  
 Hatching and pattern style  
Hatching or pattern style can be also a part of surface symbology. It’s necess ary to specify basic 
information about hatching  / pattern style in resource.xml . 130 9 – FEATURE SYMBOLOGY  
 
 
 
Implementation Guide       Example: Fill style basic information in resource.xml . 
  <ber:fillStyleArray>  
  ... 
    <ber:fillStyle id="  fs_symbol_1 " name=" patern with symbol_ no. 101"> 
      <ber:formatArray>  
...
Detailed d efinition of each style listed in resource.xml  is prepared (analogously to user line style 
...
• line style  
• line weight  IMPLEMENTATION GUIDE    VERSION 8.24 131 
...
aligned according to a tile center and it has its size and rotation. A size and rota tion is defined for a tile 
as well.  
...
 Pattern styles are implemented as cells in LIDS E dit. A cell library is on fly generated for each definition 
...
Example:  Fill style – hatch pattern . 
  ...
  <ber:crosshatchPattern id="fs_crosshatch_30_150_5">  
    ...
    <ber:lineSymbolizer>  
      <se:Stroke>  
        <se:GraphicStroke>  
132 9 – FEATURE SYMBOLOGY  
 
 
 
Implementation Guide                 <se:Graphic>  
            <se:ExternalGraphic>  
...
IMPLEMENTATION GUIDE    VERSION 8.24 133 
 
 
 
  Implementation Guide  Example:  Fill style – text Pattern.  
 
  <ber:symbolPatt ern id="fs_pattern_dr_4000">  
    <ber:grid>  
      <ber:distanceX>28</ber:distanceX>  
      <ber:distanceY>28</ber:distanceY>  
      <ber:rotation>0</ber:rotation>  
    ...
    <ber:pointSymbolizer uom="http://www.opengeospatial.org/se/units/pixel">  
      <se:Graphic>  
        <se:ExternalGraphic>  
          <se:OnlineResource xlink:href="resource.xml#ber_arimr/110/center -
center"/>  
          <se:Format/>  
        </se:ExternalGr aphic> 
        <se:Size>7</se:Size>  
...
• Opacity  – Specifies the opacity to use for rending the symbol. It has the same  semantics as th e 
...
• symbolHeight  - LIDS extension of the standar d SE point symbology. Specifies the absolute size of 
...
Example:  Symbol font basic information in resource.xml . 
 ...
 IMPLEMENTATION GUIDE    VERSION 8.24 135 
 
 
 
  Implementation Guide   Symbol fonts can be either installed in  operating system or can be stored as a resource on AS and 
downloaded only when necessary. For more details see chap ter 26.5. 
9.1.4 Text symbology  
...
subelements is supported in LIDS.  
• Font  - Identifies a font of a certain family, style, and size. The font-family  SvgP arameter gives the 
family name of a font to use. The font can be either a ny font installed in clients and server (WMS) 
operating systems  or font registered in project resources . The font-style SvgParameter gives the 
style to use for a font. The allowed val ues are normal  and italic . The font-weight  SvgParameter 
...
symbology definition).  136 9 – FEATURE SYMBOLOGY  
 
 
 
Implementation Guide       • fontHe ight – LIDS extension of the standard SE font definition. Defines the font height in the 
project base units.  
          ...
          <ber:mask>  
            ...
            <se:Stroke>  
              <se:GraphicStroke>  
                <se:Graphic>  
                  <se:ExternalGraphic>  
                    <se:OnlineRes ource xlink:href="system.linestyle#solid"/>  
                    <se:Format/>  
                  </se:ExternalGraphic>  
                </se:Graphic>  
              </se:GraphicStroke>  
...
symbology.  
Example: Displayable attr ibute symbology definition.  
  <ber:graphicTypeArray>  
    <ber:graphicType id="gtda_5012102" name="W_DA_5012102">  IMPLEMENTATION GUIDE    VERSION 8.24 137 
 
 
 
  Implementation Guide        <ber:class refId="DA"/>  
      <ber:collectionGraphicType>  
        <ber:symbology>  
          <ber:commonSymbolizer>  
            <ber:rgbColor>#1C32FF</ber:rgbColor>  
        ...
        <ber:graphicTypeComponentArray>  
          <ber:graphicTypeComponent id="gtda_5012102_2" name="Label L ine" 
                                    roleId="labelLine">  
            <ber:curveGraphicType>  
              <ber:symbology>  
                <ber:lineSymbolizer>  
        ...
        </ber:graphicTy peComponentArray>  
      </ber:collectionGraphicType>  
    </ber:graphicType>  
  </ber:graphicTypeArray>  138 9 – FEATURE SYMBOLOGY  
 
 
 
Implementation Guide       • commonSymbolizer  – Defines the symbology common for all the collection components.  
• lineSymbolizer, textSymbolizer  – Defines symbologies for collection com ponents. Color and 
...
feature symbology.  
 All symbolizers can be defined as hidde n (optional Boolean attribute).  
Example: Reference to a C ollection class Extended dimension.  
      ...
      <ber:masterGraphics nillable="false">  
        ...
        <ber:collectionGraphic Type> 
          ...
          <ber:graphicTypeComponentArray>  
            <ber:graphicTypeComponent id="gt_20" name="Dimension Text">  
              <ber:textGraphicType>  
                <ber:symbology>  
                  <ber:textSymbolizer>  
                    <se:Font>  
                      <se:SvgParameter name="fo nt-style">italic</se:SvgParameter>  
                    </se:Font>  
                    <se:Fill>  
                      <se:SvgParameter name="fill">#3F7FFF</se:SvgParameter>  
                    </se:Fill>  
                    <ber:fontHeight>1.5</ber:fontHei ght> 
                  </ber:textSymbolizer>  
                </ber:symbology>  
                <ber:defaultTextProperties>  
                  <ber:anchorPoint>center -bottom</ber:anchorPoint>  
            ...
            <ber:graphicTypeComponent id="gt_30" name="Symbol -arrow"> IMPLEMENTATION GUIDE    VERSION 8.24 139 
...
xlink:href="resource.xml#ber_LIDSdemo/50/right -center"/>  
              ...
              </ber:pointGraphi cType> 
            </ber:graphicTypeComponent>  
            <ber:graphicTypeComponent id="gt_10" name="Dimension Line">  
              <ber:curveGraphicType/>  
            </ber:graphicTypeComponent>  
          </ber:graphicTypeComponentArray>  
        </ber:co llectionGraphicType>  
      </ber:masterGraphics>  
... 
    </ber:featureType>  140 9 – FEATURE SYMBOLOGY  
 
 
 
Implementation Guide       9.2 Dynamic Symbology  
Dynamic symbology, or in other words thematic mapping, enables dynamic changes of static symbology 
according to actual view scale or values of semantic feature at tributes. The dynamic symbology is 
defined in special metadata file thematiz ation.xml . 
9.2.1 Feature type styles  
...
including co mpletely different classification into semantic classes can be defined. But only one type of 
semantic classification is allowed in one scale range.  
Features are dynamically symbolized in these steps:  
1. First of all, the scale range is chosen according to act ual view scale.  
...
with t heir static symbology.  
Scale Range 1 Scale Range M
Semantic Class 1
Attribute Condition 1
FT Symbology  1
DA Symbology 1Semantic Class N
Attribute Condition N
FT Symbology N
DA Symbology NElse Semantic Class
FT Symbology X
DA Symbology XFeature Type StyleIMPLEMENTATION GUIDE    VERSION 8.24 141 
 
 
 
  Implementation Guide  9.2.2 Semantic classification  
Depending on the types of attribute conditions there are three types of semantic classification allowed.  
General classification  
General classification allows its classes to define any semantic attributes condi tions. The conditions 
...
value s of codelist reference type attributes.  
...
(templates, selection sets) s o that it’s not necessary  to apply individual feature type styles.  
...
condition and symbolizers of appropriate types. Sti ll the logical model must be reconstructible from the 
rules.  142 9 – FEATURE SYMBOLOGY  
 
 
 
Implementation Guide       There are six types of rules:  
• Static Rule  (staticRule ) – Does not contain neither scale nor attribute condition. Just changes feature 
type static symbology with another static symbology in a feat ure type style. Cannot be combined 
...
range.  
• Interval Rule  (intervalRule )- Contains an attribute condition and optionally a scale condi tion. Used 
...
with any other rule within a scale range. Com bined semantic classification is discussed in special 
chapter 9.2.6 . 
...
• Further than 1:2000  
Main pipeline segments are not displayed in this scale range at all s o scaleOnlyRule  with 
...
Example: Dynamic symbology d efinition  (thematization.xml ). 
  <ber:featureTypeStyleArray >   
    <ber:featureTypeStyle>  
      ...
      <se:FeatureTypeName>ft_501 2100</se:FeatureTypeName>  
      <ber:intervalRule defaultScale="true">  
        <se:Title>Length[m]: 0 -5</se:Title>  
        <ogc:Filter>  
          <ogc:PropertyIsLessThanOrEqualTo>  
            <ogc:PropertyName>at_5012002</ogc:PropertyName>  
            <ogc:Literal>5</ogc:Literal>  IMPLEMENTATION GUIDE    VERSION 8.24 143 
 
 
 
  Implementation Guide            </ogc:PropertyIsLessThanOrEqualTo>  
        </ogc:Filter>  
        ...
        <ber:lineSymbolizer>  
         <se:Stroke>  
            <se:GraphicStroke>  
              <se:Graphic>  
                <se:ExternalGraphic>  
                  <se:OnlineRes ource xlink:href="resource.xml#ls_2"/>  
                  <se:Format/>  
                </se:ExternalGraphic>  
              </se:Graphic>  
            </se:GraphicStroke>  144 9 – FEATURE SYMBOLOGY  
 
 
 
Implementation Guide                   <se:SvgParameter name="stroke">#00FF00</se:SvgParameter>  
            <se:SvgParameter name="stroke -opacity">1.0</se:SvgParameter>  
            <se:SvgParameter name="stroke -width">2</se:SvgParameter>  
          </se:Stroke>  
        </ber:lineSymbolizer>  
        ...
        <ber:lineSymbolizer>  
          <se:Stroke>  
            <se:GraphicStroke>  
              <se:Graphic>  
                <se:ExternalGraphic>  
                  <se:Onli neResource xlink:href="resource.xml#ls_3"/>  
                  <se:Format/>  
                </se:ExternalGraphic>  
              </se:Graphic>  
            </se:GraphicStroke>  
      ...
      <ber:scaleOnlyRule>  
        <se:MinScaleDen ominator>1000</se:MinScaleDenominator>  
        ...
        <ber:lineSymbolizer>  
          <se:Stroke>  
            <se:GraphicStroke>  
              ...
              </se:Graphic>  IMPLEMENTATION GUIDE    VERSION 8.24 145 
 
 
 
  Implementation Guide              </se:GraphicStroke>  
            <se:SvgParameter name="stroke">#0000ff</se:Sv gParameter>  
            <se:SvgParameter name="stroke -opacity">1.0</se:SvgParameter>  
            <se:SvgParameter name="stroke -width">4</se:SvgParameter>  
          </se:Stroke>  
        </ber:lineSymbolizer>  
      </ber:scaleOnlyRule>  
      <ber:scaleOnlyRu le hiddenScale="true">  
...
9.2.5 Graphic tags dynamic symbology  
...
Example: Displayable attributes dynamic symbology definition.  
      <ber:inte rvalRule id="rule_456_1" name="Length[m]: 0 -5"> 
        <ogc:Filter>  
          <ogc:PropertyIsBetween>  
            <ogc:PropertyName>at_5012002</ogc:PropertyName>  
            <ogc:LowerBoundary>  
            ...
            <ogc:UpperBoundary>  
              <ogc:Literal>5</ogc:Literal>  146 9 – FEATURE SYMBOLOGY  
 
 
 
Implementation Guide                   </ogc:UpperBoundary>  
          </ogc:PropertyIsBetween>  
        </ogc:Filter>  
        <ber:lineSymbolizer>  
          <se:Stroke>  
...
is necessary to identify t he graphic tag type and the component type. For this reason, PropertyName  
inside Geometry  element is present in graphic tag dynamic symbolizer.  
9.2.6 Combined semantic classification  
Combined semantic classification enables independent  resymbolization of partic ular symbology 
...
symbology properties according to different semantic attributes and different classification types.  IMPLEMENTATION GUIDE    VERSION 8.24 147 
 
 
 
  Implementation Guide  For any symbology property, one of these definitions can be used:  
...
• discrete classification  
• general classification  
 If a discrete classification is used in a combined classificatio n, just one attribute can be used. It is a 
...
• general classification i s used  
...
<ber:featureTypeStyle id="fs_321"  
  ...
  <ber:description>Main Pipeline Segment - hide + combin ed</ber:description>  
  ...
  <ber:combinedRule id="rule_80000"  
    ...
    <se:MaxScaleDenominator>1000 </se:MaxScaleDenominator>  
    <ber:lineSymbolizer>  
      <se:Stroke>  148 9 – FEATURE SYMBOLOGY  
...
Implementation Guide               <se:GraphicStroke>  
        ...
        <se:SvgParameter name="stroke">  
          <ber:recode fal lbackValue="#888888">  
          <!-- color changed by discrete classification -->    
            ...
            </ber:mapItem>  IMPLEMENTATION GUIDE    VERSION 8.24 149 
 
 
 
  Implementation Guide              <ber:mapItem>  
              <ber:data>3</ber:data>  
            ...
            <ber:elseClass>  
              <ber:value id="fsv_16">#8888 88</ber:value>  
            </ber:elseClass>  
          </ber:recode>  
        </se:SvgParameter>  
...
• tresholdsBelongTo  - Defines whether the split po ints belong to a preceding or succeeding (default) 
interval.  
...
elseClass  rule definition as it cannot have ID.  150 9 – FEATURE SYMBOLOGY  
 
 
 
Implementation Guide       9.2.7 Symbology tokens  
Symbology tokens are values of already evaluated classification s for feature type style s. Symbology 
...
 The tokens are needed  by LIDS Applic ation server when rendering the features using dynamic 
...
- style s referred as dynamicSymbology  in feature t ype definition in model.xml  
- themes which include attribute  utilizedForWMS =”true”  in thematization.xml  
This means, e very feature can have symbology tokens from more styles stored in the feature table . 
 Symbology tokens can be used to optimize dynamic symbolo gy performance also in LIDS Edit / 
...
attributes in orde r to be able to evaluate the dynami symbology classification. This is time consuming. 
If the symbology tokens exist for particular dynamic symbology definition, client application can apply 
the symbology using the symbology tokens  directly  and it’s not nec essary to download also the 
attributes. This improves the performance of downloading the graphics significantly.  
Following modes of d ownloading graphics into LIDS Edit / Explorer  can occur : 
...
• dynamic symbology withou t symbology tokens – classification has to be evaluated by the client 
application, semantics has to be downloaded to the clients  additionally to the geometry  
 ...
 Chapter 10 
 Containers  
 IMPLEMENTATION GUIDE    VERSION 8.24 153 
 
 
 
  Implementation Guide  10. Containers  
...
• Semantic t able – Contains all semantic attributes defined for a group of feature types.  
...
defined for the feature typ e and all his parents, the feature type must have geometry. These feature types 
are usually used as some kind of graphic marks. Features of this feature type is stored in feature and 
graphic tables.  
 
Semantic
sid,at1,at2,at3, ...Feature
fid,sid,sec1, ...Graphic
fid,gid,sdo_geom , ...
Semantic
sid,at1,at2,at3, ...Feature
fid,sid,sec1, ...
Feature
fid,sid, ...Graphic
fid,gid,sdo_geom,...154 10 – CONTAINERS  
 
 
 
Implementation Guide       10.4 Defining Feature Containers  
...
• history - changes  are recorded  for keep ing history  
• longTransaction  - changes are done in long transaction mode  
• measure  – Boolean. If true, graphical table stores also m-coordinate of individual vertexes . IMPLEMENTATION GUIDE    VERSION 8.24 155 
...
It’s not possible to store a geometry which exceeds the range . Still, t he range value should be set 
reasonably according to project data. If set too large, many spatial operations don’t work correct . If 
z range is defined, also z coordinate s are stored for features in this container, enabling 3D 
functionality  
...
querying many smaller tables.  156 10 – CONTAINERS  
 
 
 
Implementation Guide        Which feature types will be grouped into which group containers is really the project designer choice. 
...
<ber:model>  
 ...
 IMPLEMENTATION GUIDE    VERSION 8.24 157 
 
 
 
  Implementation Guide      <ber:container id="ct_5010005" name="Hydrant" dbName="W_HYDRANT">  
  ...
  <ber:featureTypeArray>  
    ...
    <ber:featureType id="ft_5010200" name="Extraction point"  
                     parentId="ft_5010000" abstract="false">  
      <ber:description>Extraction point</ber:descr iption> 
...
• dbName  – Name of a group contain er database table.  
• versioned  – Optional parameter. Possible values:  
• flase  – default. No versioning is activated  
• history – changes  are recorded  for keep ing history . Value true can still be used because of 
backwards compatibility and has same meaning   
• longTransaction  - changes are done in long transaction mode  
• measure  – Boolean. If true, graphical table stores also m-coordinate of individual vertexes.  158 10 – CONTAINERS  
...
containers definition . 
 ...
 IMPLEMENTATION GUIDE    VERSION 8.24 161 
 
 
 
  Implementation Guide  11. Categories  and G roups  
...
zero. Furt hermore, categories can be arranged in a tree structure. Such organization enables to logically 
categorize feature types according to various aspects, which results in e ffective feature type handling.  
...
Example : Definition o f categories.  
<ber:categoryArray>  
  ...
  <ber:category  id="cat_5020000" name="Electrical network">  
    ...
    <ber:featureIdArray > 162 11 – CATEGORIES  AND GROUPS  
 
 
 
Implementation Guide             <ber:item refId="ft_5020000"/>  
...
• parentI d – References its super category in the backdrop category hierarchy. Optional.  
Inner Elements  
• description  – Contains a detailed description of the group container.  
• backdropTypeIdArray  – Collection of item elements, i.e. backdrop types contained in this ca tegory.  IMPLEMENTATION GUIDE    VERSION 8.24 163 
 
 
 
  Implementation Guide  Example: Backdrop category structure.  
...
11.3 Groups  
Groups are more general than categories. While categories are limited to feature types on ly, groups can 
contain feature  types, codelist s, association s and reports . 
...
<ber:featureType id="ft_1" name=" Abc"> 
  ...
  <ber:container refId="ct_ 1"/> 
  <ber:featureAttributeArray>  
… 164 11 – CATEGORIES  AND GROUPS  
 
 
 
Implementation Guide       Example: Assigining association  to two groups . 
<ber:relationAssoc dbName="R_ A" id="r_ 1" name=" Abc"> 
  <ber:groupsArray>  
    <ber:group id=" group_1"/> 
    <ber:group id=" group_2"/> 
  </ber:groupsArray>  
  <ber:srcRole refId="rt_ 1"/> 
  <ber:destRole refId="rt_ 2"/> 
</ber:relationAssoc>  
...
group: group search  includes all feature types include d in search.xml , group collaboration -comments  
...
well.  
 Content of one or more groups can be evaluated by exe cuting following REST request (individual 
...
http://server:port/project/rest/metadata/group/business,collaboration -comments,search  
 ...
 Chapter 12 
 Backdrop Maps  
 IMPLEMENTATION GUIDE    VERSION 8.24 167 
 
 
 
  Implementation Guide  12. Backdrop Maps  
...
binaryRasterBackdropType ). Every b ackdrop map has its definition (backdrop type) in the metadata. 
...
of a particular layer need not create a continuo us coverage.  
...
backdrop maps). Every backdrop t ype can define one default static and one default dynamic symbology. 168 12 – BACKDROP MAPS 
 
 
 
Implementation Guide       While the static symbology is the part of the backdrop map definition, the dynamic symbology refers 
special LIDS metadata section (in thematization.xml ). 
 
Example:  Vector backdrop map def inition.  
<ber:backdropTypeArray>  
  <!-- More detailed vector backdrop --> 
  <ber:vectorBackdropType id="bd_01" name="Overview map">  
    <ber:description>More detailed vector backdrop</ber:description>  
    <ber:layerArray>  
      <ber:tiledLayer id="lay_bd_0 1_dgn" name="DGN overview map"  
                      format="dgn"/>  
...
<ber:vectorBackdropStyle id="bs_01" name="Colored by level and geometry type">  
  <!—Visibility accord ing to scales --> 
  <ber:scaleRules hiddenDefault="false">  
    <ber:scaleOnlyRule hiddenScale="true">  
      <se:MinScaleDenominator>500</se:MinScaleDenominator>  
      <se:MaxScaleDenominator>2000</se:MaxScaleDenominator>  
    </ber:scaleOnlyRule>  
  </ber:sc aleRules>  IMPLEMENTATION GUIDE    VERSION 8.24 169 
 
 
 
  Implementation Guide    <!— Classification rules --> 
...
• Literal  – Possible values i n case of PropertyName  = element.type: Point, Ellipse, EllipseArc, Line, 
Polyline, Multiline, Polygon, Spline, Text, TextNode, LineString, ComplexString, Dimension, 
Shape, ComplexShape, Cell.  170 12 – BACKDROP MAPS 
 
 
 
Implementation Guide       12.2 Raster Backdrop Maps  
Raster backdrop type can have several layers. Each layer has format and structure. Supported raster 
formats are TIFF, JPEG, PNG, CIT, COT, HMR , BM P and GIF . Raster backdrop layers can have tiled 
...
of backdrop type - binaryRasterBackdropType . It allows t o define color and transparency for both 
foreground and background.  
Example: Raster backdrop map definition.  
<ber:backdropTypeArray>  
  <ber:rasterBackdropType id="bd_02" name="Orthophoto">  
    <ber:description>Hierarchical orthophoto raster</ber:descriptio n> 
    <ber:layerArray>  
  ...
  <ber:binaryRasterBackdropType id="bd_06" name="Monochromatic city map">  
    ...
    </ber:layerArray>  IMPLEMENTATION GUIDE    VERSION 8.24 171 
...
sizes and can overlap. Tiles of a particular layer need not create a continuous coverage. This stru cture 
...
Hierarchical layers  
The layer is represented by files which cover the particular area i n the hierarchy of various details. The 
...
more det ailed data (the pyramid base). The pyramidal structure is built from original data (main tiled 
...
• tileWidth  – width of a single tile in a hierarchy (recommended value is 10 24 px) 
• tileHeight  – height of a single tile in a hierarchy (recommended value is 10 24 px) 
• decreaseRatio  – number of subordinate tiles in X axis which is the superior tile gene rated from 
...
• maxParallelRequests  – Maximum number of requests a single client can execute in parallel.  172 12 – BACKDROP MAPS 
 
 
 
Implementation Guide       12.2.1  Dynamic symbology  
Dynamic symbology is defined in the thematization.xml  metadata par t. It consists of 2 parts:  
...
<ber:binaryRasterBackdropStyle id="bts_301" name="Gray foreground">  
 ...
 IMPLEMENTATION GUIDE    VERSION 8.24 175 
 
 
 
  Implementation Guide  13. Overview Maps  
...
backdrop maps with a spatial restriction. Different spatia l areas of the same backdrop map may, thus, 
...
<ber:vicinityMapArray>  
  ...
  <ber:vicinityMap id="vi_02" name="Overviewmap" default="false">  
    <ber:descr iption>Overviewmap</ber:description>  
    <ber:range>  
      <ber:x min="3449000" max="3464400"/>  
      <ber:y min="5476300" max="5490500"/>  
    </ber:range>  
  ...
  <ber:vicinityMap id="vi_03" name="Overviewmap - zoom" default="false">  
    ...
    </ber:range>  176 13 – OVERVIEW MAPS 
 
 
 
Implementation Guide           <ber:backdropTypeRefs>  
      <ber:backdropType refId="bd_01"/>  
    </ber:backdropTypeRefs>  
  </ber:vicinityMap>  
...
• backdropTypeRefs  – References the set of backdrop maps.  
 ...
 Chapter 14  
 Attachments  
 IMPLEMENTATION GUIDE    VERSION 8.24 179 
 
 
  Implementation Guide  14. Attachments  
...
'filesystem'.  
• secured  – Specifies whether or not the access rights are applied fo r an attachment.  
...
storage="database" secured="true">  
    ...
    <ber:attachmentType id="att_pu mpingstation_f" name="Pumping station - file 
...
application server file system</ber:descript ion> 
    </ber:attachmentType>  
...
storage="database" secured="false" georeferenced="true">  
      ...
      <ber:featureTy pe refId="ft_g_attachment"/>  
    </ber:attachmentType>  180 14 – ATTACHMENTS  
 
 
 
Implementation Guide           <ber:attachmentType id="att_general_f_att" name="General attachment - FS + att" 
storage="filesystem" georeferenced="true">  
      <ber:description>Georeferenced picture with attributes stored on app lication 
server file system</ber:description>  
      <ber:featureType refId="ft_g_attachment"/>  
    </ber:attachmentType>  
  </ber:attachmentTypeArray>  
14.1 Versioning  attachments  
The attachments can maintain historical versions. New versions are generated automa tically when 
...
feature type is defined as versioned, its attachment s are versioned as well.  
...
these tables is 1:1. But theoretically there can be more records in the LIDS_DOCUMENT _H table 
related to one labelled version in LIDS_DOC_VERSION.  
 ...
 IMPLEMENTATION GUIDE    VERSION 8.24 183 
 
 
 
  Implementation Guide  15. Displayable Attributes  
...
1. it is necessary to define the graphic t ype class  
2. graphic t ype is defined with the reference to the graphic t ype class  
...
Example:  Definition of graphic t ype class.  
    <ber:graphicTypeClassArray>  
      <ber:graphicTypeClass id="DA" name="Displayable attribute">  
        <ber:description>Displayable attributes, graphic tag</ber:description>  
      </ber:graphicTypeClass>  
...
Example:  Definition of graphic t ype. 
            ...
            <ber:curveGraphicType>  
              <ber:symbology>  
                <ber:lineSymbolizer>  
                  <se:Stroke>  
                    <se:SvgParameter name="stroke -width">1</se:SvgParameter>  
                  </se:Stroke>  184 15 – DISPLAYABLE ATTRIBUTES  
 
 
 
Implementation Guide                       </ber:lineSymbolizer>  
              </ber:symbology>  
            </ber:curveGraphicType>  
          </ber:graphicTypeCompone nt> 
          <ber:graphicTypeComponent id="gtda_5012102_1" name="DA Text" 
roleId="text">  
            <ber:textGraphicType>  
              <ber:symbology>  
              ...
              <ber:defaultTextProperties>  
                <ber:anchorPoint>le ft-top</ber:anchorPoint>  
              </ber:defaultTextProperties>  
            </ber:textGraphicType>  
          </ber:graphicTypeComponent>  
        </ber:graphicTypeComponentArray>  
...
Example:  Definit ion of graphic tag type  
<ber:graphicTagTypeArray>  
...
for main pipeline seg ments. Its instance can look like this:  
 IMPLEMENTATION GUIDE    VERSION 8.24 185 
...
• id – Unique identifie r of a displayable attribute type.  
• name  – Name of a displayable attribute type.  
186 15 – DISPLAYABLE ATTRIBUTES  
 
 
 
Implementation Guide       • description  – Description of a displayable attribute type.  
• graphicTagRefArray  – References to graphic tag types that represent this displayable attribute type.  
• daQuery  – Repres ents the query used for deriving displayable attribute value. Similarly to derived 
...
borderLine .  
If ignoreMasterRotation  attribute is set to true , the distance in the X box is det ermined in the 
...
 You can use similar functions like in case of derived attribut es, including geometric (see chapter 4.5.3 ). 
...
attribute. In this case, it usually has more li nes so it is called multiline displayable attribute. Generally, 
multiline displayable attributes consist of two parts. First part defines the first line – information from 
master feature. The second part defines the subsequent lines of a displayable attrib ute. They are gathered 
...
the number of lines is not the main difference between simple and multiline displayable attributes.  IMPLEMENTATION GUIDE    VERSION 8.24 187 
 
 
 
  Implementation Guide  Example: Multiline displayable attributes definition. Consists of the main identification followed by 
list of subordinate valves. Its instance can look like this:  
 
<ber:displayableAttrib ute id="da_5012103" name="W_DA_5012103">  
  <ber:graphicTagRefArray>  
...
features.  
• relationAssoc  – Relation t hat connects subordinate features to master feature.  
...
Compound displayable attributes are s pecial kind of displayable attributes. They consist of individual 
...
levels. They don’t need to related to the master feature directly. The level of rel ationship can be 
...
Example:  Definition of graphic t ype. 
<ber:gra phicType id="gtcda_01" name="Compound DA1">  
    ...
    <ber:graphicTypeComponentArray>  
      <ber:graphicTypeComponent id="gtcda_01_route" name="Route" roleId="DA">  
        <ber:textGraphicType>  
          ...
          <ber:defaultTextProperties>  
            <ber:anchorPoint>left -bottom</ber:anchorPoint>  IMPLEMENTATION GUIDE    VERSION 8.24 189 
 
 
 
  Implementation Guide            </ber:defaultTextProperties>  
        </ber:textGraphicType>  
      </ber:graphicTypeComponent>  
      ...
      <ber:delegateGraphicTypeComponent id="gtcda_01_c 1" name=" Cable 1" roleId="DA">  
        <ber:textGraphicType>  
          <ber:symbology>  
          </ber:symbology>  
          <ber:defaultTextProperties>  
        ...
        <ber:featureTypeDelegate ref Id="ft_cable _1"/> 
      </ber:delegateGraphicTypeComponent>  
      <ber:delegateGraphicTypeComponent id="gtcda_01_c 2" name="Cable 2" roleId="DA">  
        <ber:textGraphicType>  
          <ber:symbology>  
          </ber:symbology>  
          <ber:defaultTextPr operties>  
            <ber:anchorPoint>left -bottom</ber:anchorPoint>  
          </ber:defaultTextProperties>  
        </ber:textGraphicType>  
        <ber:featureTypeDelegate refId="ft_cable_ 2"/> 
      </ber:delegateGraphicTypeComponent>  
      <ber:delegateGr aphicTypeComponent id="gtcda_01_pps" name=" Duct" roleId="DA">  
        <ber:textGraphicType>  
          <ber:symbology>  
          </ber:symbology>  
          <ber:defaultTextProperties>  
            <ber:anchorPoint>left -bottom</ber:anchorPoint>  190 15 – DISPLAYABLE ATTRIBUTES  
...
Implementation Guide                 </ber:defaultTextProperties>  
      ...
      <ber:graphicTypeComponent id="gtcda_01_ll" name="Label line" 
roleId="labelLine">  
        <ber:curveGraphicType>  
          <ber:symbology>  
          </ber:symbology>  
...
<ber:compoundDisplayableAttributeArray>  
  <ber:compoundDisplayableAttribute id="da_ducts_cables" name="Route ,ducts,cables">  
    ...
    <ber:daQuery>  
      ...
      <ber:compoundQu ery> IMPLEMENTATION GUIDE    VERSION 8.24 191 
  ...
  Implementation Guide          <ber:attributeArray>  
        ...
        <ber:layout lineHeight="1.2" level Offset="0.0"/>  
        <ber:compoundQuery>  
          <ber:attributeArray>  
            <ber:attribute xpath="at_5120005"/>  
          </ber:attributeArray>  
          <ber:format>*{0}*</ber:format>  
          <ber:relationAssoc xlink:href="model.xml#r_duct_cab le"/> 
      ...
      <ber:compoundQuery>  
        ...
        <ber:layout l ineHeight="1.2" levelOffset="0.0"/>  
      </ber:compoundQuery>  
    </ber:daQuery>  
    <ber:daAutoConstruction>  
      <ber:position>  
      ...
      </ber:position>  192 15 – DISPLAYABLE ATTRIBUTES  
 
 
 
Implementation Guide             <ber:angleFromMaster/>  
    </ber:daAutoConstruction>  
...
be excluded  
 ...
 Chapter 16 
 Associations  
 IMPLEMENTATION GUIDE    VERSION 8.24 195 
 
 
 
  Implementation Guide  16. Associations  
...
An association type contains roles. An association type can contain one or more identified role t ypes 
...
Destination Role. A rela tion type contains, as all element types in a project, identifier, name, description 
and the database name.  
The definition of relation types is situated within the assocTypeArray  XML collection. Role types are 
defined within the roleTypeArray  XML collectio n. 
...
<ber:model>  
  ...
  <ber:roleTypeArray>  
    ...
    <ber:relationRole id="rt_ mains" name="Water mains"  
          dbName=" MAINS"> 
      <ber:ftItem refId="ft_ mains"/> 
    </ber:relationRole>  
    <!-- Define the destination role with feature types inside --> 
    <ber:relationRole id="rt_ pipe" name="Main pipeline segment"  
         dbName=" PIPE"> 
      <ber:ftItem refId="ft_ pipe_a"/> 
      <ber:ftItem refId="ft_ pipe_b"/>    </ber:relationRole>  
  </ber:roleTypeArray>  
 
  <ber:assocTypeArray>  
    <!-- Define the relation between Mains and Main Pipelines --> 
    <ber:relationAssoc id="r_ mains_pipes"  
       name="Mains --&gt; Main pipeline se gment" dbName="R_ MAINS_PIPES "> 
      <!-- The Source role reference --> 
      <ber:srcRole refId="rt_ mains" cardinality="1"/>  
      <!-- The Destination role reference --> 
      <ber:destRole refId="rt_ pipe"/> 
    </ber:relationAssoc>  
  </ber:assocTypeArray>  
</ber:model>  
Association Type:
...
Role Type:
Water mainsRole Type:
Main pipeline segment
Feature Type:
Water mainsFeature Type:
Main pipeline segmentIMPLEMENTATION GUIDE    VERSION 8.24 197 
 
 
 
  Implementation Guide  16.1.1  Role Type Definition  
...
The role type definition contains the followin g information:  
...
The interfaceR ole type definition contains the following information:  
...
attribute for all feature types in the same container  198 16 – ASSOCIATIONS  
 
 
 
Implementation Guide       • Derived attribute using relationQuery on inter face role must define interface attributes instead of 
feature attributes in the attributeArray  
• Interface role can’t be combined with spatialRelation  
16.1.2  Relation Type Definition  
...
dbName="R_SCH_CABLE_AXIS">  
  ...
  <ber:destRole refId="rt_e_route_sch_ d" cardinality="1"/>  
</ber:relationAssoc>  
Attributes  
...
transactions too. The containers in the other roles of such relations have to b e versioned either for 
...
feature(s) in source/destina tion role will be deleted during destination/source feature deletion.  IMPLEMENTATION GUIDE    VERSION 8.24 199 
 
 
 
  Implementation Guide  16.1.3  Spatial relations  
There can be a spatial relation defined between features. The spatial relation is based on spatial 
interaction of two features; it is managed automatically by system. T he spatial relation is defined by 
...
• interact ionType  – Defines interaction condition.  
...
• lineToLine  – Represents a line to line spatial interaction with the given spatial condition on parallel 
lines. This interaction considers master -slave relationship and resolves whether the slave line meets 
spatial condition to the master line. The parameter spatialCondi tion defines whether the lines are:  
• partialParallel  – The whole destination line is parallel to a part of a source line . 
• fullParallel  – The two lines are parallel i n the whole length . 
• parallel  – They are either fully parallel or partially parallel. Further more, it is irrelevant which 
...
• the surface anyhow contains (either inside or on the boundary) t he whole line ( contains ). 
...
• the so urce surface contains the whole destination surface ( contains ). 
...
Implementation Guide       Example:  Defining spatial relation between main pipeline segments and valves.  
<ber:model>  
  ... 
  <ber:roleTypeArray>  
...
pipe --&gt; Protection pipe" dbName="SR_5012000">  
      ...
      <ber:interactionType>  
        <ber:lineToLine spatialCondition="partialParallel"/>  IMPLEMENTATION GUIDE    VERSION 8.24 201 
 
 
 
  Implementation Guide          <ber:roleFilter refId="rt_5012300_d">  
          <ber:featureType refId="ft_5012300"/>  
        </ber:roleFilter>  
      </ber:interactionType>  
    </ber:spatialRelationAssoc>  
16.1.4  Relation constraints  
...
<ber:condition r1.roleId="rx"  r2.roleId="r y" r1.ftids="ft_1 ft_2"  r2.ftids="ft_3" > 
  {r1.at_1} != 'AAA' 
</ber:condition>  
...
• r1.attribute , r2.attribu te – optional definition. It enables filter the features the condition / constraint 
is applied to  
...
• operators  – currently supported operators include: =, !=, <, >, <=, >=  
...
using default values first and opens the form for specifying the attributes in the next step. In such case, 202 16 – ASSOCIATIONS  
 
 
 
Implementation Guide       the default values might violate the constraint, so t he creation would not be possible at all.  
...
YELLOW, ot herwise the relation can’t be created  
  <ber:rule id="" name=""  cascadeRollback=" true"> 
    <ber:constraint r1.roleId="rt_ 1">{r1.at_color} = 'YELLOW'</ber:constraint>  
  </ber:rule>  
...
YELLOW. If the material is not GOLD, the rule is not evaluated  
  <ber:rule id="" name="">  
    <ber:condition r1.roleId="rt_ 1">{r1.at_material} = 'GOLD'</ber:condition>  
...
on role 2 must be YELLOW.  
  <ber:rule id="" name="">  
    <ber:condition r1.roleId="rt_ 1">{r1.at_material} = 'GOLD'</ber:condition>  
    <ber:constraint r1.roleId="rt_ 2">{r1.at_color} = 'YELLOW'</ber:constraint>  
  </ber:rule>  
...
the feature in role 2 must be feature type 2  
  <ber:rule id="" name="">  
    <ber:condition r1.roleId="rt_ 1" r1.ftids="ft_1"/>  
    <ber:constraint r1.roleId="rt_ 2" r1.ftids="ft_ 2"/> 
  </ber:rule>  
 
Example:  Rule based on feature type and attribute conditions . If feature type is 1 or 2  and the material 
is GOLD , the related feature type has to be 3 or 4 and the color has to be YELLOW   
  <ber:rule id="" name="">  
    <ber:condition r1.roleId="rt_ 1" r1.ftids="ft_1 ft_2">{r1.at_material}  = 'GOLD' 
    </ber:condition>  
    <ber:constraint r1.roleId="rt_ 2" r1.ftids="ft_ 3 ft_4">{r1.at_color} = 'YELLOW'  
    </ber:constraint>  
  </ber:rule>  
 IMPLEMENTATION GUIDE    VERSION 8.24 203 
 
 
 
  Implementation Guide  Example:  Rule using more constraints. All must be met. If feature type is 1 or 2  and the material is 
...
null, t he color attributes of related feature s have to be the same   
  <ber:rule id="" name="">  
    <ber:condition r1.roleId="rt _1">{r1.at_material} != NULL</ber:condition>  
    <ber:constraint r1.roleId="rt_ 1" r2.roleId="rt_ 2"> 
...
<ber:featureRefRelationAssoc id="r_1" name=" Pipe --> Inspection">  
  ...
  <ber:childRole refId="rt_inspection_d" deleteWit hMasterFeature="true"/>  
  <ber:relationRuleArray>  
    <ber:rule id="rc_ 1" name=" …"> 
...
ft_pig_inspection"/>  
      <ber:constraint r1.roleId="rt_ pipe">{r1.at_diameter} != -1</ber:constraint>  
    </ber:rule>  
  </ber:relationRuleArray>  
</ber:featureRefRelationAssoc>  204 16 – ASSOCIATIONS  
 
 
 
Implementation Guide       16.2 Details  
...
Example:  Defining the detail association for the pumping station feature ty pe. 
<ber:model>  
  ... 
  <ber:roleTypeArray>  
    ...
    <ber:detailRole id="r t_5090000_det" name="Detail - detail feature">  
...
Role Type:
Pumping station 
- master featureRole Type:
Detail - detail feature
Feature Type:
Pumping stationFeature Type:
Conductor
Feature Type:
BusbarFeature Type:
SwitchFeature Type:
OutletFeature Type:
FuseIMPLEMENTATION GUIDE    VERSION 8.24 205 
 
 
 
  Implementation Guide      </ber:detailRole>  
  </ber:roleTypeArray>  
  <ber:assocTypeArray>  
...
deleted automatically when deleting the detail associatio n. 
    <ber:detailAssoc id="det_1" name=" ABC"> 
      <ber:description> Detail content automatically deleted </ber:description>  
      <ber:masterRole refId="rt_ 1_mst"/> 
      <ber:detailRole refId="rt_ 1_det" deleteWithDetail="true" /> 
    </ber:detailAssoc>  
...
Implementation Guide        
 ...
 IMPLEMENTATION GUIDE    VERSION 8.24 209 
 
 
 
  Implementation Guide  17. Topology  
...
that can act as nodes in the linear graph are included in the nodeRole. Similarly, all fe ature types that 
...
topology graph as it is unimportant. On the other hand the directe d linear graph has all edges directed. 
Every edge in the directed graph has start node and end node.  
17.1.1  Graph creation  
Graph elements – nodes and edges are derived from standard features ’ geometry . The graph elements 
are created  / updated / deleted  when sourc e features are created , updated / deleted.  Connection between 
...
Point features  
Features with point geometry can b e used to create nodes. Usually, one node feature is represented by a 
single node in the topology graph.  
Following limitation s exist:  
• Two nodes of the same graph can’t exist on the same position.  It’s not possible to construct two 
point features defined a s nodes of the same graph on the same coordinates.  
...
correct topology connection to the topology edges. The definition fo r splitting edge features into many 
...
situation can only happen when node have a line -string geom etry. For feature vertexes topology nodes 
...
System node  is  generated automatica lly on edge end , if there is nostandard node  in the specified 
distance . Typically, system nodes are generated after constructing linear features away from  point 
features.  210 17 – TOPOLOGY  
 
 
 
Implement ation Guide       When point feature is created on the position where system node exists, the system no de is replaced by 
the standard one.  
...
<ber:linearTopologyAssoc xmlns:ber="http://www.berit.com/ber"  
  ...
  <ber:nodeRole id="ast_0 1_node" name="Node of Water graph">  
...
edgeRole
Feature Type:
Extraction PointFeature Type:
ValveFeature Type:
Service PipeIMPLEMENTATION GUIDE    VERSION 8.24 211 
...
graph to be updated only in a batch mode. The online flag allows batch upd ating also.  
Node Role  
...
element  212 17 – TOPOLOGY  
 
 
 
Implement ation Guide        Attributes  
• id – Unique id entifier of the node role.  
...
nodes in the graph. Cannot be emp ty. 
Point Symbolizer  
...
a one line -string feature there will be only one edge created.  
 Inner elements  
• featureType  – List of feature type references. All referenced fe ature types can be topology 
edges in the graph. Cannot be empty.  IMPLEMENTATION GUIDE    VERSION 8.24 213 
 
 
 
  Implementation Guide  Line Symbolizer  
Defines symbology for graph edges visualization.  
17.1.3  Directed graph  
...
undirected.  214 17 – TOPOLOGY  
 
 
 
Implement ation Guide       Undirected constraints  
• Node Constraint  – Defines p ossible feature types in the edge role which a feature type in the node 
...
• Node Constrai nt – For a feature type in the node role defines a set of feature types in the edge role 
...
Node constraints are defined inside the nodeType  element. Edge constraint s are defined inside the 
edgeType  element.  
17.1.5  Constraints for undirected graph  
Node Constraints  
Node constraints for undirected graph are defined in node types.  
 
Example:  Definition of n ode constraints.  
<ber:nodeRole id="ast_01_node" name="Node of Water graph">  
  <ber:nodeType systemEdge="none">  
 ...
 IMPLEMENTATION GUIDE    VERSION 8.24 215 
 
 
 
  Implementation Guide  Constraint  
...
Edge Constraints  
Edge constraints for undirected graph are defin ed in edge types.  
...
possible feature types in the node role the edge can be conne cted to. If the list is empty then this edge 
...
Inner elements  
• featureType  – List of feature type references. All referenced feature types can be connected to the 
node. When no feature type is referenc ed then this edge cannot be connected to any feature node.  216 17 – TOPOLOGY  
 
 
 
Implement ation Guide       17.1.6  Constraints for directed graph  
...
System
NodeExtraction
PointIMPLEMENTATION GUIDE    VERSION 8.24 217 
 
 
 
  Implementation Guide        <ber:directedConstraint > 
        <ber:outEdge> 
          <ber:featureType refId="ft_5012200"/> <! -- Service Pipe --> 
        </ber:outEdge> 
      </ber:directedConstraint > 
...
empty it means that no feature edges can be incomi ng to the node.  
...
Edge Constraints  
Edge constraints for directed graph defines possib le start nodes and end nodes for an edge type. Edge 
constraints are defined inside the edgeType  definition.  
 
Example: Definition of edge constraint  
<ber:edgeRole id="ast_01_edge" name="Edge of Water graph">  
  <ber:edgeType systemNode="terminal" processVert ices="terminal">  
    <ber:featureType refId="ft_5012200"/> <! -- Service Pipe --> 
      ...
      <ber:directedConstraint > 
        ...
        <ber:endNode allowsSystemNode="true">  
           <!-- allowsSystemNode="true" -> the edge can  
                start in a system node --> 
          <ber:featureType refId="ft_5011100"/>  <! -- Valve --> 218 17 – TOPOLOGY  
 
 
 
Implement ation Guide                 <ber:featureType refId="ft_5010200"/> <! -- Extraction Point --> 
        </ber:endNode> 
      </ber:directedConstraint > 
    </ber:directedConstraintArray > 
  </ber:edgeType>  
...
including system nodes. If the endNode  element i s missing, it means that the edge can end in all 
...
<ber:linearDir ectedTopologyAssoc  
...
Point
System
NodeIMPLEMENTATION GUIDE    VERSION 8.24 219 
...
Implement ation Guide               <ber:directedConstraint>  
          <ber:startNode allowsSystemNode="true">  
            <!-- Service Pi pe can start in a system node --> 
          </ber:startNode>  
...
17.1.7  Bridges  
Bridges are nongraphical, topological only elements used for connecting standard ele ments of linear 
graph. Bridges are considered during tracing functionality and enable tracing to pass through elements 
which are not geometrically connected. Bridges can connect either two nodes or two edges o f the same 
or different graphs.  Apart from node s and edges, bridges are not derived from the source features 
...
• startRole and endRole ftItem  – feature type c onnected by  the starting or ending node of the bridge  
• lineSymbolizer  – symbology used to display bridge in the client application  
 
Example:  Definition of edge to edge bridge  
<ber:linearTopologyConnection id="ltc_1" name=" Pipeline segments brige" 
type="edgeToEdge">  
  ...
  <ber:lineSymbolizer>  
    <se:Stroke>  IMPLEMENTATION GUIDE    VERSION 8.24 221 
 
 
 
  Implementation Guide        <se:SvgParameter name="stroke">#880000</se:SvgParameter>  
      <se:SvgParameter name="stroke -width">1</se:SvgParameter>  
    </se:Stroke>  
  </ber:lineSymbolizer>  
...
<ber:linearTopologyConnection id="ltc_2" name="Station - Valve bridge" 
type="nodeToNode">  
  <ber:startRole id="start_r_asltc_2" name="Station role">  
    <ber:ftItem refId="ft_5010100"/>  
  </ber:startRole>  
  <ber:endRole id="end_r_asltc_ 2" name="Valve role">  
    <ber:ftItem refId="ft_5011100"/>  
  </ber:endRole>  
  <ber:lineSymbolizer>  
    <se:Stroke>  
...
Implement ation Guide        
 ...
 Chapter 1 8 
 Reports  
 IMPLEMENTATION GUIDE    VERSION 8.24 225 
 
 
  Implementation Guide  18. Reports  
...
The report definitions are store d as a JasperReports templates.  
...
• The user may select a particular report template within client applicatio n including output format of 
the response.  
...
engine.  
• The data returned from storage is used by LIDS Ap plication Server to populate the report.  
...
XMLReport Design
Document
DocumentPDF, HTML, XLS, 
RTF, TXT JRXML226 18 – REPORTS  
...
Report type  Dynamic  MasterDetail  WFS based  SQL based  
staticReport  No No Yes No 
dynamicReport  Yes No Yes No 
masterDetailReport  No Yes Yes No 
dynamicMasterDetailReport  Yes Yes Yes No 
jasperReport  No Yes No Yes 
dynamicJasperReport  Yes Yes No Yes 
18.2.1  SQL based report data source  
The data source for reporting is specified bywriting SQL statement directly.  
18.2.2  WFS based r eport data source  
...
910
2
345IMPLEMENTATION GUIDE    VERSION 8.24 227 
...
18.2.4  Dynamic reports  
The repor t template is dynamically modified by the request sent from client as well as query that selects 
the data. User usually selects a list of features from client application and sends a report request for these 
features. The report template is modified to inc lude feature specific attributes, custom title etc.  
 
This type of reports are very complex and difficult to create. Jasper report templates for these reports 
should be designed to be able to visualize all kinds of feature types and unlimited number of attr ibute 
...
18.2.5  Master –detail reports  
Master –detail reports are spec ial kind of static reports that contains detail section for every master 
...
informations for each report template. Gen erally, these metadata tells the reporting service the name of 
...
18.3.1  Dynamic reports  
Dynam ic reports are identified by dynamicReport  element followed by report identifier, its display able 
...
If the report request does not specify output format then default one is taken.  228 18 – REPORTS  
 
 
 
Implementation Guide       • inputParameterArra y contains a list of inputParameter  elements. Users may set these parameters 
...
Example : Dynamic report c onfiguration ( presentation.xml ). 
<ber:reportArray>  
...
default="true">  
  ...
  <ber:inputParameterArray>  
    ...
    <ber:inputParameter id="DATE 2" name=" Some date "> 
      <ber:description> Date for filtering the data </ber:description>  
      <ber:defaultValue>  IMPLEMENTATION GUIDE    VERSION 8.24 229 
 
 
  Implementation Guide          <ber:currentDate/>  
      </ber:defaultValue>  
      <ber:dataType>  
        <ber:date/>  
      </ber:dataType>  
    ...
    <ber:inputParameter id="YEAR" name="Report Year"  nillable="false" > 
      ...
      <ber:defau ltValue>2013</ber:defaultValue>  
      <ber:dataType>  
...
18.3.2  Relative path to resources  
...
<parameter name="RESOURCE_ROOT" isForPrompting="false" class="java.lang.String"/>  230 18 – REPORTS  
 
 
 
Implementation Guide       Example of usage:  
<imageExpression><![CDATA[$P{RESOURCE_ROOT}+"/report/ file.jpg"]]></imageE xpression>  
...
18.5.1  Creating report template using JasperSoft Studio  
The iReport is capable of querying multiple data sourc es including databases using JDBC driver. 
...
IMPLEMENTATION GUIDE    VERSION 8.24 231 
...
3. In the following Data Source  dialo g select your Data Adapter created before.  
 
IMPLEMENTATION GUIDE    VERSION 8.24 233 
...
Implementation Guide       6. Next step allows you to group fi elds. 
...
definition and finally Preview mode ge nerates preview of the report filled with actual data.  In Preview 
...
If you choose predefined template for your repo rt, you will probably need to make some adjustments . If 
...
IMPLEMENTATION GUIDE    VERSION 8.24 235 
...
• Page Header – header of pages; the page header appears on all pri nted pages in the same position  
...
be independently visible or not) different properti es are associated with a group.  
• Group Footer – the group footer band completes a group; usually it contains fields to view 
subtotal s or separation graphic elements, such as lines.  
...
• Summary – The summary band allows to insert fields concerning total calculations, means, or 
whatever you want to i nsert at the end of the report.  
...
IMPLEMENTATION GUIDE    VERSION 8.24 237 
 
 
  Implementation Guide     
 
You can ei ther remove some of bands o r add another. By changing their width you define the space 
...
panel (in Appearance  tab) or with using of graphical el ements in Design  panel . You can also remove 
...
Implementation Guide       You can add Basic and Composite Elements to layout from Palette  panel (up -right) by drag and drop. 
...
field element an d sets the text field expression for that element to $F{NAME}. This text can be further 
...
them from t he list in Fields section in Outline panel to the main window to Detail  section. Automatically 
there will be also headers created in Column  header  section.  
For each report element is possible to set many properties. The best way how to set or edit properti es is 
...
IMPLEMENTATION GUIDE    VERSION 8.24 239 
 
 
  Implementation Guide  Inserting co mpany logo  
...
If you want to define conditiona l style first create style and then from context menu over created 
style choose Create conditional style . Then it’s necessary to define conditon via Expression Editor 
dialog (e.g. $F{ATTRIBUTE1}=0  ) and finally you can define or adjust style characteristic s via 
Properties panel.  
...
1.  Choose Table  from Basic Elements  in Pallete  and drag it to the section of your choice  (typically 
to some Detail band) .  
 
IMPLEMENTATION GUIDE    VERSION 8.24 241 
 
 
  Implementation Guide  2. The following dialog appears . You will probably have to choose creating table using a new 
dataset . Click on Next button.  
 
 
3. Enter name for new dataset (you can use the na me predefined by system). Click on Next button.  
 
 
4. Select the Data Adapter and then d efinition of the query is same as when creating a report (see 
...
for individual table sections, style for cell borders etc. Finish whole procedure by  click ing on 
Finish  button . The table will be added to the selected band.  
 
IMPLEMENTATION GUIDE    VERSION 8.24 243 
 
 
  Implementation Guide  6. You can get the similar result as on picture below. The ta ble element is placed to selected band 
with choosen colour schema and with pr edefined size and position. In Outline panel new 
“dataset” and “table” elements are added automatically as well.  
 
7. Tuning of all table and table cells properties is possible by do uble click on given table. Design tab 
...
Implementation Guide       8. Example report with 4 tables (datasets), each placed in separate de tail band you can see on picture 
below.  
...
FROM table    
WHERE table .fk_attribute  = $P{input_parameter_f or_interconnection } …. 
  where $P{input_parameter_f or_interconnection } is parameter defined under given Dataset   
• Then it is necessary to define “mapping” of parameters. For this operation you can use these ste ps: 
...
picture below.  
IMPLEMENTATION GUIDE    VERSION 8.24 245 
...
• Specify connection  
• Add dataset parameters – add parameter(s) used within query f or interconnection between subreport and 
main report  
...
• Find el ement Subreport in given Detail band and click on it to display Properties panel  
...
If this type of report is requesting on the ser ver, in the request are definied WFS queries. In report filling 
...
IMPLEMENTATION GUIDE    VERSION 8.24 247 
...
18.5.2  Registering report inside LIDS Application Server  
...
needed to accomplish th is task.  
...
Implementation Guide       Example:  Static report (Blank_A4_jasper.jrxml ) from JasperSoft Studio.  
http://jasperreports.sourceforge.net/xsd/jasperreport.xsd" name="Blank_A4 _jasper" 
...
topMargin="20" bottomMargin="20" uuid="6360915a -5381-42a6-9553-dec75020a510">  
 ...
 <property name="com.jasper soft.studio.data.defaultdataadapter" 
value="LIDSDEMO"/>  
...
name="Blank_A4 _jasper_dyn " pageWidth="595" pageHeight="842" columnWidth="555" 
leftMargin="20" rightMargin="20" topMargin="20" bottomMargin="20" uuid="6360915a -
5381-42a6-9553-dec75020a510">  
 <property name="com.jaspersoft.studio.data.sql.tables" value=""/>  
 <property name="com.jaspersoft.studio.data.defaultdataadapter" value="One 
Empty Record"/>  
 <parameter name="SID_LIST" class="java.util.Collection">  
  <defaultValueExp ression><![CDATA[]]></defaultValueExpression>  
 </parameter>  
 <queryString language="SQL">  IMPLEMENTATION GUIDE    VERSION 8.24 249 
 
 
  Implementation Guide    <![CDATA[SELECT *  
FROM "W_PUMPSTAT"  
WHERE $X{IN,sid,SID_LIST}]]>  
 </queryString>  
 <field name="X_COORD" class="java.math.BigDecimal"/>  
...
Example:  Registration of static report Blank_A4_jasper.jrxml  inside presenta tion.xml . 
    <ber:jasperReport id="jrp_tibco" name="Test TIBCO Jasper" 
orientation="portrait">  
      <ber:description>report in jasper format</ber:description>  
      <ber:outputFormatArray>  
        <ber:outputFormat default="true">pdf</ber:o utputFormat>  
        <ber:outputFormat>html</ber:outputFormat>  
        <ber:outputFormat>csv</ber:outputFormat>  
        <ber:outputFormat>xls</ber:outputFormat>  
      </ber:outputFormatArray>  
...
orientation="portrait">  
      ...
      <ber:templateName>Blank_A4_jasper_dyn.jrxml</ber:templateName>  250 18 – REPORTS  
 
 
 
Implementation Guide             <ber:featureTypeArray>  
        <ber:featureType xlin k:href="model.xml#ft_5010100"/>  
      </ber:featureTypeArray>  
...
iReport 5.6.0 . You have to rep lace database names with metadata identifications of these attributes. 
...
*/<at_identifier>/<codelist_display_column>  forma t. 
...
After you are done with report customizations place the JasperReports template (.jrxml file) inside 
Resources/Report  directory of the LIDS Application Server.  
The last thing that needs to be done is to register a newly created report inside project Present ation.xml  
file. These reports are labelled as <ber:staticReport> , <ber:dynamicReport> , 
...
orientation="portrait">  
      ...
      <ber:outputFormatArray>  
        <ber:outputFormat default="true">pdf</ber:outputFormat>  
        <ber:outputFormat>html</ber:outputF ormat> 
        <ber:outputFormat>csv</ber:outputFormat>  
        <ber:outputFormat>xls</ber:outputFormat>  
      </ber:outputFormatArray>  IMPLEMENTATION GUIDE    VERSION 8.24 251 
 
 
  Implementation Guide        <ber:templateName>my_report.jrxml</ber:templateName>  
      <ber:reportQueryArray>  
        <ber:reportQuery>  
          ...
          <ber:attributeArray>  
            ...
            <ber:attribute xlink:href="mod el.xml#at_5012102"/>  
          </ber:attributeArray>  
          <ogc:Filter>  
            <ogc:PropertyIsBetween>  
              <ogc:PropertyName>at_5012102</ogc:PropertyName>  
...
Implementation Guide        
 ...
 IMPLEMENTATION GUIDE    VERSION 8.24 255 
 
 
  Implementation Guide  19. Tools  
It’s possible to define set of project specific tool s using the tool.xml  definition. These tools are available 
...
metadata file and stored on AS in the appropriate subdirectories of the ${project folder}lids -
as/resources/icon/bmp16x16 directory.  
 
 Detailed description of all possi ble modules and functions which can be defined in tools is included 
...
<ber:toolBoxArray > 
  ...
  <ber:toolBox id="tb_01" name=" Main toolbox "> 
    ...
    <ber:subToolArray > 
      <ber:tool refId="ti_ 1"/> 
      <ber:tool refId="ti_ 2"/> 
      <ber:tool refId="ti_ 3"/> 
      <ber:separator/>  
      <ber:tool refId="ti_ 4"/> 
      <ber:tool refId="ti_ 5"/> 
      <ber:separator/>  256 19 – TOOLS  
 
 
 
Implementation Guide             <ber:tool refId="ti_ 6"/> 
    </ber:subToolArray > 
  </ber:toolBox> 
  ...
  <ber:toolBox id="tb_011" name=" Sub-toolbox 1"> 
    ...
    <ber:subToolArray > 
      <ber:tool refId="ti_ 10"/> 
      <ber:tool refId="ti_ 11"/> 
      <ber:tool refId="ti_ 12"/> 
      <ber:tool refId="ti_ 13"/> 
    </ber:subToolArray > 
  </ber:toolBox> 
  ...
  <ber:toolBox id="tb_012" name="  Sub-toolbox 2 "> 
    ...
    <ber:superToolBox  refId="tb_01"/>  
    <ber:subToolArray > 
      <ber:tool refId="ti_ 20"/> 
      <ber:tool refId="ti_ 21"/> 
      <ber:tool refId="ti_ 22"/> 
    </ber:subToolArray > 
  </ber:toolBox> 
</ber:toolBoxArray > 
...
19.2 Ribbon Pages  
LIDS Explorer enables selection of the  menu style as ribbon. In this case the project tools d efinition in 
...
• ribbonPage  – defines the  main section in the ribbon  IMPLEMENTATION GUIDE    VERSION 8.24 257 
 
 
  Implementation Guide  • ribbonPageGroup  – section inside a ribbonPage with individual tools and separators  
...
parameter, the tools are displayed in more co lumns  
...
<ber:ribbonPageArray>  
  <ber:ribbonPage id="rp_p1" name="Page 1"> 
    <ber:ribbonPageGroup id="rpg_g1" name="Group 1"> 
       <ber:tool refId="ti_ 1" style="Large" label=" Tool 1"/> 
       <ber:tool refId="ti_ 2" style="SmallWithText" label=" Tool 2"/> 
       <ber:separator/>  
       <ber:tool refId=" ti_3"/> 
    </ber:ribbonPageGroup>  
  ...
  <ber:ribbonPage  id="rp_p2" name="Page 2"> 
    <ber:ribbonPageGroup  id="rpg_g3" > 
       <ber:tool refId="ti_ 5" style=" SmallWithText "/> 
       <ber:tool refId="ti_ 6" style="SmallWithoutText"/>  
    </ber:ribbonPageGroup>  
  </ber:ribbonPage>  
</ber:ribbonPag eArray> 
...
Implementation Guide        
 ...
 IMPLEMENTATION GUIDE    VERSION 8.24 261 
 
 
  Implementation Guide  20. Display Model  
...
Order of selected templates, saved data selections, backdrop maps and references to ex ternal data inside 
...
• for each feature type its visibi lity (to draw it or not),  
...
references to cross section - an appropriate data will be plotted by assi gned display model.  
20.1 Tips  
...
• enter the template with surface feature types to the end of display mode l so surface features will 
...
feature . 
Proposed solution:  
If a displayable attribute DA1 of feature type F1 should „j ump over“ another feature types (contained 
...
Implementation Guide        
 ...
 Chapter 21  
 Profile  IMPLEMENT ATION GUIDE    VERSION 8.24 265 
 
 
  Implementation Guide  21. Profile  
...
elements.  Metadata extension has def inition in XML (profile .xml) and for user defined templates is used 
...
Implementation Guide        
 ...
 IMPLEMENTATION GUIDE    VERSION 8.24 269 
 
 
  Implementation Guide  22. LIDS Edit / Explorer  
22.1 explorer.xml  
...
<ber:modelExplorer xmlns:ber="http://www.berit.com/ber"  
  ...
  <ber:version>  
    <ber:metadata>1</b er:metadata>  
    <ber:minClient> 7676</ber:minClient>  
    <ber:minAS>1 71001</ber:minAS>  
  </ber:version>  
  <ber:groupArray>  
    <group key="businessMetaGroup" value=" explorerB usiness,requests "/> 
...
boAssignFillAttributes="true"  copyAfterAssetSplit="true" > 
      <ber:featureType xlink:href="model.xml#ft_ bo_assign"/>  
      <ber:relationAssoc xlink:href="model.xml#as_ bo_assign "/> 
      <ber:assetRelationAssocArray>  
        <ber:relationAssoc xlink:href="model.xml#as_ bo_asset_1"/> 
        <ber:relationAssoc xlink:href="model.xml#as_ bo_asset_2"/> 
        <ber:relationAssoc xlink:href="model.xml#as_ bo_asset_3"/> 
      </ber:assetRelat ionAssocArray>  
    </businessAssignment>  
  </businessAssignmentArray>  
  <ber:dynamicDAArray>  
    <ber:graphicTagType xlink:href="model.xml#ft_ street_da_name"/>  
  </ber:dynamicDAArray>  
  <ber:parameterArray>  
    <ber:parameter name="as_timeout" value="66666 6"/> 
    <ber:parameter name="automatic_download_relations" value="true"/>  
    <ber:parameter name="explorer_detail_max_scale" value="35"/>  270 22 – LIDS  EDIT / EXPLORER  
 
 
 
Implementation Guide           <ber:parameter name="uStn_respect_backdrop_GO" value="true"/>  
  ...
  <ber:dependentFormArray>  
    <ber:dependentForm>  
      <ber:featureType xlink:href="model.xml#ft_ A"/> 
      <ber:attribute xlink:href="model.xml#at_ 01/ca_01"/> 
    </ber:dependentForm>  
  </ber:dependentFormArray>  
  <ber:onDemandDownloadRelationTypeArray>  
    <ber:relationAssoc xlink:href="model.xml# as_1"> 
      <ber:relationRole xlink:href="model.xml#rt_ 2"/> 
    </ber:relationAssoc>  
  </ber:onDemandDownloadRelationTypeArray>  
</ber:modelE xplorer>  
 
• businessMetaGroup  – comma separated name s of metadata group s. Forms of feature types from 
these group s only will include possibility to call SAMO business actions . If value “ business ” is used, 
forms of all feature types which have a SAMO business action defined will be expanded by the 
appropriate function s. The group business  is a system group, it doesn’t have to be defined in 
model.xml. If  businessMetaGroup  definition is missing, no SAMO business actions are available in 
LIDS Edit / Explo rer 
• searchMetaGroup  – comma  separated name s of metadata group s. If defined, only feature types 
from th ese group s will be queried by the function Search out. If not defined, all feature types defined 
...
types from these groups can’t be s elected as source or destination for the feature type change 
function. In case of shared semantics,  any feature type included in this group causes a semantic 
parent and all graphical children to be excluded  IMPLEMENTATION GUIDE    VERSION 8.24 271 
 
 
  Implementation Guide  • businessAssignmentArray  – section for defining th e SAMO assignment  
• assetAssignFillAttributes  – true / false attribute defines, whether the fo rm for filling the 
...
• dynamicDAArray  – list of displayable attribute types, which will be moved automatically to f it into 
map frame  
...
• onDeman dDownloadRelationType  – defines list of relations + roles, whose content will not be 
...
22.1.1  Paramete rs 
The f ollowing parameters were included in config.xml  and were editable by LIDS Application server 
...
Time limit ( sec) for i nactivity of LIDS Explorer / LIDS Edit clients. If no communication between 
the client application and the server takes place in the defined interval, a dialog box is displayed 
for the user with a 30 s countdown. Once this has expired, the client applicati on will be 
...
Switches on/off option Automatic committing of operations  after logg ing in to the project.  272 22 – LIDS  EDIT / EXPLORER  
 
 
 
Implementation Guide       • backdropCache_dir ( directory path ) 
...
Manages downloading of codelists on tr ansfer to off -line mode (all codelists / only codelists 
...
Maximu m allowed scale for window displaying a detail.  
...
Flag defining if global origin of DGN files is consid ered when attaching backdrop maps. Default 
is false.  
...
• project_info_time  
The parameter specifi es the timestamp for the webpage defined in the project_info  parameter. It 
...
setting types include e.g. useroptions, projectoptions,keybo ardshortcuts, guicustomized etc. The 
folder has to be created manually and the existing settings should be moved there  IMPLEMENTATION GUIDE    VERSION 8.24 273 
 
 
  Implementation Guide  • coordinatesScale  (whole number >= 0 ) 
Defines the number of fraction digits for displaying coordinates in some windows. In LIDS 8.22 
limit ed to the window Change Coordinates - Scale  
...
Manages use of SID or FID identifiers for req uests sent from TOMS to LIDS AS.  
...
The client application includes functions for managing tw o types of option settings:  
...
every user.  
 274 22 – LIDS  EDIT / EXPLORER  
 
 
 
Implementation Guide       The client application always generates complete user options setting includ ing all possible parameters 
...
• If some parameter exists in the user options wi th the flag changed="True" , the value from user 
...
• Set the val ue of particular entry in the user options to changed="False" . It will be updated according 
...
3. Colors for background, se lection set and highlight are set according to project / user Options  
...
6. Project fonts are i nitialized in the file  
...
The function fo r copying features can operate in two modes. The new feature s either take over the type 
of the source feature or they can be assigned another type on the basis of rules saved in metadata . IMPLEMENTATION GUIDE    VERSION 8.24 275 
 
 
  Implementation Guide  The rules are defined in option.xml  in featureCopyArray . Every rule  can defin e following properties:   
...
Example : featureCopy definitions in option.xml  
  <ber:featureCopyArray>  
    <ber:featureCopy  id="fc_1" name="Map A converted to map B and transformed">  
      <ber:parameters rotation="180" scaleFactor="1.5" horizontalFlip="true" 
verticalFlip="true"/>  
      <ber:copyFTArray>  
    ...
    <ber:featureCopy id=" fc_2" name=" ..."> 
      <ber:parameters rotation="0" scaleFactor="1.0" horizontalFlip="false" 
verticalFlip="false"/>  
      <ber:copyFTArray>  
        <ber:copyFT sharedSemantic="true">  
          <ber:inputFT xlink:href="model.xml#ft_ 1"/> 
          <ber:outputFT xlink:h ref="model.xml#ft_ 1"/> 
        </ber:copyFT>  
        <ber:copyFT sharedSemantic="true">  
          <ber:inputFT xlink:href="model.xml#ft_ 2"/> 
          <ber:outputFT xlink:href="model.xml#ft_ 3"/> 
        </ber:copyFT>  276 22 – LIDS  EDIT / EXPLORER  
 
 
 
Implementation Guide             </ber:copyFTArray>  
    </ber:featureCopy>  
  </ber:featureCopyArray>  
 ...
 Chapter 2 3 
 LIDS Browser  
 IMPLEMENTATION GUIDE    VERSION 8.24 279 
 
 
  Implementation Guide  23. LIDS Browser  
...
defines used data layers. Legends are raster images representin g „static“ legends with symbology.  
Example:  Structure of browser.xml . 
 ...
 <ber:locationForms> ber:L ocationFormsType </ber:locationForms> [0..1]  
 ...
 <ber:def aultUserSettings> ber:DefaultUserSettingsType 
 ...
 <ber:timeout> xsd:de cimal </ber:timeout> [0..1]  
 <ber:embedit> xsd:boolean </ber:embedit> [0..1]  
 <ber:areaMeasurement> ber:AreaMeasurementType </ber:areaMeasurement> [0..1]  
 <ber:mapContextCache> xsd:boolean </ber:mapContextCache> [0..1]  280 23 – LIDS  BROWSER  
 
 
 
Implementation Guide        <ber:backdropLayers> ber:BackdropLay ersType </ber:backdropLayers> [0..1]  
...
1. Upload browser.xml – LIDS AS Admin console => Project Control => Metadata ext ension Files 
=> LIDS Browser  
...
• locationSymbology  – Symbology properties f or element location (general  / for specified feature 
types).  
• stampText , usageLink  – Text content for stamp (company name, version etc.) and for link to Terms.  
  <ber:mainMap>  
    <ber:initialSettings>  
      <ber:range> IMPLEMENTATION GUIDE    VERSION 8.24 281 
 
 
  Implementation Guide          <ber:x min="3451400.00" max="3451850.00"/>  
        <ber:y min="5481600.00" max="5481900.00"/>  
      </ber:range>  
    </ber:initialSettings>  
    <ber:minScaleDenominator>100</ber:minScaleDenominator>  
    <ber:maxScaleDenominator>500000</ber:maxScaleDenomina tor> 
    <ber:scales>  
    ...
    <ber:locationSymbology>  
      ...
      </ber:featureTypeSymbolizerArra y> 
    </ber:locationSymbology>  
    <ber:stampText>© 2016 Asseco CE</ber:stampText>  
    <ber:usageLink usageLinkUrl="http://asseco.com/ce/">Terms of 
Usage</ber:usageLink>  
  </ber:mainMap>  282 23 – LIDS  BROWSER  
 
 
 
Implementation Guide       23.2 Overview Map 
Name of registered backdrop layer which represents over view (vicinity) map and initial range from 
...
LIDS_SETTINGS  table), backdrop  (defined in model.xml ), TMS layer  (defined in map-
service.xml ) and external layer  (identifier of external server, stored in LIDS store).  
...
• minScaleDenominator /maxScaleDen ominator  – The lowest and the highest map scale for 
downloading data.  
Layer grouping  
...
minScaleDenominator ="100" maxScaleDenominator ="2000"/>  
    <ber:layer refId="setdefinitio n_201105021258"/>  
    <!-- Hierarchy modeled using nonexpandable master category  --> 
    <ber:layer refId=" cat_redlining_sketch "/> IMPLEMENTATION GUIDE    VERSION 8.24 283 
 
 
  Implementation Guide      <!-- Hierarchy modeled using folder --> 
...
Example:  
  <ber:externalWMS>  
    <ber:externalServers>  
      ...
      <ber:externalServer xlink:href="http://brno -
as1.berit.cz:9200/LIDSdemo_DEV/WebMapServ ice"/> 
    </ber:externalServers>  
    <ber:allowedHosts>  
...
in GetFeature  request as :variable_name:  (e.g. from example below „variable1 “). 284 23 – LIDS  BROWSER  
 
 
 
Implementation Guide       Request  subelement  
...
– true/ false;  codelist: clId – codelist identifier; keyId  – codelist code attri bute identifier; displayId  – 
codelist d escription attribute identifier).  
...
outputFormat="WMS_GML">  
          <wfs:Query typeName="ft_506 0000"> 
            <ogc:Filter>  
      ...
      <ber:form>  
        <table xmlns="http://www.w3.org/1999/xhtml">  
          ...
          </tr> IMPLEMENTATION GUIDE    VERSION 8.24 285 
...
• refId  – List of related layers (category, set definition, backdro p, tms) for which the given legend is 
displayed.  
...
• noteAttribute  – Refers the feature attribute for inserting the text value. This eleme nt is optional. If 
this element is missing then LIDS Browser insert only geometry.  
• layer  – Refer s a category or setting containing this feature type.  
• showMeasurement – Enables displaying of length / surface of the drawn line / polygon.  
• snapp ingLayers  – Defines layers  for snapping during drawing.  
...
Example: Redline task.  
    ...
    <ber:group name="Sketch">  
      <ber:redlineTask>  
        ...
        <ber:noteAttribute  xlink:href="model.xml #at_5080003"/>  286 23 – LIDS  BROWSER  
 
 
 
Implementation Guide               <ber:layer refId="cat_5080000"/>  
      </ber:redlineTask>  
...
If assoc sArray  element is defined, the feature without graphics is created only as the related feature to 
an existing one.  
 
Example:  
  <ber:insertFeatur eTypeArray>  
    <ber:featureType xlink:href="model.xml#ft_ 1"/> 
    <ber:featureType xlink:href="model.xml#ft_ 2"/> 
    <ber:featureType xlink:href="model.xml#ft_ 3"> 
      <ber:assocsArray>  
        <ber:assoc id="r_ 1" srcFt="ft_ 4" srcRole="rt_ 1" destRole="rt_ 2"/> 
      </ber:assocsArray>  
    </ber:featureType>  
  </ber:insertFeatureTypeArray>  IMPLEMENTATION GUIDE    VERSION 8.24 287 
 
 
  Implementation Guide  23.9 Plot Layouts  
Contains list of plot settings. It should contain plottinglayout settings and default plot layout. List of 
predefined plot layouts (creating is p ossible in LIDS Explorer) which can be used in LIDS Browser. 
...
map layout and for its individual componen ts.  
• scales  – List of possible map scales which can be used for given plot layout.  
• rotations  – List of possible rotation s which can be used for given plot layout  (from -180 to 180) . 
 LIDS Browser supports only single -page plot layouts with following compone nts: 
...
Example:  Plot layout.  
  ...
  </ber:plotLayouts>  288 23 – LIDS  BROWSER  
 
 
 
Implementation Guide       23.10  Default Settings  
...
• maxFeaturesMapSelected – defines maximum number of features sent to for m when selecting 
...
Example:  Default user settings.  
  <ber:defaultUserSettings>  
    ...
    <ber:snappingModes>  
     ...
     <ber:snappi ngMode id="edge">true</ber:snappingMode>  
...
Example:  Plugin.  
    <ber:plugin id="addressPoint">  
      <ber:configuration>  
        <ber:forms>  IMPLEMENTATION GUIDE    VERSION 8.24 289 
 
 
  Implementation Guide            <ber:form xlink:href="model.xml#fmx_50121"/>  
        </ber:forms>  
      </ber:configuration>  
    </ber:plugin>  
23.12  GUI Configuration  
Metadata d efinition of graphical user interface . If defined according the following example, interface 
will look like  this: 
...
Implementation Guide               <ber:module id="views"/>  
...
IMPLEMENTATION GUIDE    VERSION 8.24 291 
 
 
  Implementation Guide          <ber:toolbarItem type="button" id="hand"/>  
      ...
      <ber:applicationToolbar>  
        ...
        <ber:toolbarItem type="button" id="a bout"/> 
      </ber:applicationToolbar>  
    </ber:toolbars>  
...
Implementation Guide             <ber:paintingPanelItem id="delete"/>  
...
Example:  Visualisation style.  
    <ber:styles>  
      <ber:style id="mainGradientFro m">#F2F7FF</ber:style>  
...
IMPLEMENTATION GUIDE    VERSION 8.24 293 
 
 
  Implementation Guide  Example:  Symbology of vector graphics.  
...
23.14  Additional L ayers  
The graphical data displaying in the map is mainly based on loadi ng the layers in the form of raster 
...
needs).Onl y the data from explicitly defined layers are loaded in the vector form  parallel to the rasters , 
...
Example:  Definition of additional layers . 
  <ber:featureTooltipInfoLayers>  
    <ber:layer refId="setdefinition_ 1"/> 
    <ber:layer refId="s etdefinition_ 2"/> 
    <ber:layer refId="setdefinition_ 3"/> 
  </ber:featureTooltipInfoLayers>  
  <ber:selectByPolygonLayers>  
    <ber:layer refId="setdefinition_ 2"/> 
...
Implementation Guide       Example:  Topological tracing tasks . 
  <ber:topoTracingArray>  
    <ber:traceOut xli nk:href="option.xml#tr_1"/>  
    <ber:traceOut xlink:href="option.xml#tr_2"/>  
    <ber:traceOut xlink:href="option.xml#tr_3"/>  
  </ber:topoTracingArray>  
23.16  Contexts  
Configuration for LIDS Browser used for some named context = initialization of LIDS Browser fro m 
...
 Calling LIDS Browser with use of CONTE XT parameter:  
...
• <ber:mapConte xtCache>  True/false.  
...
• <ber:featureBrowse><ber:pageSize>  Number of records wi th alphanumerical data per page, 
<ber:exportMax>  Maximum si ze of export  – decimal.  
...
• <ber:embedit>  Boolean. True to run LIDS Browser in embedded mode  in IFRAME.   
 ...
 Chapter 2 4 
 LIDS Mobile  
 IMPLEMENTATION GUIDE    VERSION 8.24 297 
 
 
  Implementation Guide  24. LIDS Mobile  
...
Privileges and access rights are controlled the same way as in LIDS by LIDS Security. LIDS Set tings 
...
If these forms are not defined, all attributes ar e used. For the features list the first three attributes are 
used.  
...
5481750.8</ber:initialSituation>  
  ...
  <ber:serverArray>  
    <ber:server name="LIDS WMS" defaultTurnOn="false" sho wAsOneLayer="false" 
...
"> 
      <ber:layerArray>  
        ...
        <ber:layer id="l_2 " name="" defaultTurnOn="false"/>  298 24 – LIDS  MOBILE  
 
 
 
Implementation Guide               <ber:layer id="l_3" name="" defaultTurnOn="false"/>  
      </ber:layerArray>  
    </ber:server>  
...
xlink:href="WMTS?SERVICE=WMTS&amp;VERSION=1.0.0&amp;REQUEST=GetCapabilities">  
  ...
  <ber:groupArray>  
    <group key="searchMetaGrou p" value=" group_id "/> 
  </ber:groupArray>  
  <ber:offlineSearchArray>  
    <ber:indexedFeatureTypes>  
      <ber:indexedFeatureType xlink:href="model.xml#ft_ 1"/> 
      <ber:indexedFeatureType xlink:href="model.xml#ft_ 2"/> 
      <ber:indexedFeatureType xlink:href="model.xml#ft_ 3"/> 
    </ber:indexedFeatureTypes>  
    <ber:indexedFeatureAttributes>  
      <ber:indexedFeatureAttribute xlink:href="model.xml#at_ 1"/> 
      <ber:indexedFeatureAttribute xlink:href="model.xml#at_ 2"/> 
      <ber:indexedFeatureAttribute xlink:href="model.xml#at_ 3"/> 
      <ber:indexedFeatureAttribute xlink:href="model.xml#at_ 4"/> 
    </ber:indexedFeatureAttributes>  IMPLEMENTATION GUIDE    VERSION 8.24 299 
 
 
  Implementation Guide    </ber:offlineSearchArray>  
</ber:modelMobile>   
24.1 Project info  
...
• zoomLevel – optional  default  zoom level according to 
...
from LIDS project or from external map services. Individual layers are sorted by groups called server s. 
 
• name  – name displayed in the application  
• defaultT urnOn  – defines, if the layer is turned on by default  
• showAsOneLayer – if true, it’s not possible to display list of individual layers in client application  
• defaultQueryLayer – if true, the layer is used for selecting features in graphics by default . Sever al 
layer s can be set to true . Default value is true . 
...
o symbol ogy evaluated by application server  
o can be used in offline mode, but a lot of raster tiles can be downloaded  300 24 – LIDS  MOBILE  
 
 
 
Implementation Guide       • LIDS -VECTOR  
o good for frequently changed data  
o symbology limited - evaluated by client  
o optimal for offline mode  
• WMS , WMTS  
o Used for accessing external map sevic es 
• featureInfoOutput – format of feature info; used only for external WMS / WMT S 
...
element . If value  of parameter is excluded, the parameter can be defined for e ach layer 
independently  
Example:  
  <ber:serverArray>  
...
networks" showAsOneLayer="false" type="LIDS -VECTOR" context= "utility"> 
      <ber:layerArray>  
...
xlink:href="htt p://server.com/map?SERVICE=WMS&amp;REQUEST=GetCapabilities&amp;VERSI
ON=1.1.1">  
      <ber:layerArray>  
      ...
      </ber:layerArray>  IMPLEMENTATION GUIDE    VERSION 8.24 301 
 
 
  Implementation Guide        <ber:paramsArray>  
...
• groupName  – optional attribute; defi nes the collapsible group under which the list is displayed  
...
• featureTypeArray  – mandatory definition of feature typ es to be loaded  
• condition  – optional attribute filter . The syntax is the same as in case of featureBrowse  tool 
• orderBy  – optional definition of attributes for default ordering  302 24 – LIDS  MOBILE  
 
 
 
Implementation Guide       • orderByAttribute  – optional   list of attributes to be available for interactive ordering for this browse ; 
...
<ber:browseArray>  
  <ber:browse id="b1" name="List 1" groupName="Group 1" pageSize="50">  
    ...
    <ber:filterAttributeArray>  
      <ber:filterAttribut e xlink:href="model.xml#ft_01/at_02"/>  
...
'createdBy' = '%%USERID%%'</be r:condition>  
      </ber:filter>  
      <ber:orderByArray>  
        <ber:orderBy direction="descending" xlink:href="model.xml#at_01"/>  IMPLEMENTATION GUIDE    VERSION 8.24 303 
 
 
  Implementation Guide          <ber:orderBy xlink:href="model.xml#ft_02"/>  
      </ber:orderByArray>  
    </ber:dataSource>  
    <ber:orderByAttribu teArray>  
      <ber:orderByAttribute xlink:href="model.xml#ft_01/at_02"/>  
      <ber:orderByAttribute xlink:href="model.xml#ft_01/at_03"/>  
    </ber:orderByAttributeArray>  
    <ber:filterAttributeArray>  
      <ber:filterAttribute xlink:href="model.xml#ft_0 1/at_02"/>  
      <ber:filterAttribute xlink:href="model.xml#ft_01/at_04"/>  
    </ber:filterAttributeArray>  
    <ber:groupByAttribute xlink:href="model.xml#ft_01/at_05"/>  
  </ber:browse>  
...
relationView Meta Group  in groupArray . More comma separated metadata groups can be defined  in this 
way: 
• if relationView Meta Group  definition doesn’t exist, all relationship types are displayed  
• if relationView Meta Group  definition exists, only the relationship types from these groups are 
displayed  
24.4.1  Relationships in off line 
...
Example:  
  <ber:sketchTaskArray>  
    <ber:group name="Group1">  
      <ber:sketchTask>  
        <ber:featureTyp e xlink:href="model.xml#ft_1"/>  304 24 – LIDS  MOBILE  
 
 
 
Implementation Guide               <ber:featureType xlink:href="model.xml#ft_2"/>  
        <!—Example of feature type with shared semantics --> 
        <ber:featureType xlink:href="model.xml#ft_ s3/ft_3"/> 
      </ber:sketchTask>  
    </ber:group>  
...
24.6 Search Configuration  
List of feature types queried by LIDS Mobile can be limited by specifying comma separated metadata 
group s ids in group Array of mobile.xml . If this configuration exists, only feature types included in the 
specified group s will be evaluated during search  
Example:  
  <ber:groupArray>  
    <group key="searchMetaGroup" value=" group_1,group_2 "/> 
  </ber:groupArray>  
 
If this definition doesn’t exist, a ll indexed feature types will be evaluated  during search.  
24.6.1  Search extension configuration for LIDS Mobile  
...
only. The other attributes in the detail form are downloaded from LIDS AS directly, so they do n't need 
to be indexed.  
...
Attributes need ed to be included in the index:  
• "type"  
• "id" 
• "featureInfo"  
• attributes needed for evaluating the query  
Indexing of feature geometry  and indexing of codelists is not necessary for LIDS Mobile.  IMPLEMENTATION GUIDE    VERSION 8.24 305 
 
 
  Implementation Guide  24.7 Offline Search  
LIDS Mobile is capable of search ing data according to attribute filter also in offline mode. Such data 
...
attribute search in offline mode.  Only the specif ied attributes of the specified feature types are indexed.  
Example:  
  <ber:offlineSearchArray>  
    <ber:indexedFeatureTypes>  
      <ber:indexedFeatureType xlink:href="model.xml#ft_ 1"/> 
...
24.9 Project tools  
It’s possible to define two types of project tools to be used in LIDS Mobile: tools and featureTools.  
• tools – functions are available directly in the application menu  
• featureTools – functions are available in the form of particular feature  306 24 – LIDS  MOBILE  
 
 
 
Implementation Guide       Both tools definition s consists of:  
• name  
• icon – reference to resources  
...
24.9.3  Construct new related f eature  
...
captured feature  yes 
roleType  Relation role of the to -be-captured feature  yes 
featureType  To-be-captured feature type  yes 308 24 – LIDS  MOBILE  
...
Implementation Guide       Example:  
  <ber:featureTool name="New inspection">  
    ...
    <ber:featureTypeArray>  
      ...
      <ber:featureType xlink:href="model.xml#ft_bus_st op"/> 
    </ber:featureTypeArray>  
    <ber:script>  
      <ber:function id="constructNewRelatedFeature"/>  
      <ber:parameterArray>  
        <ber:parameter name="relationType" value="r_house_inspection"/>  
        <ber:parameter name="roleType" value="rt_ins pection"/>  
        <ber:parameter name="featureType" value="ft_inspection"/>  
      </ber:parameterArray>  
    </ber:script>  
  </ber:featureTool>  
24.9.4  Display related features in the list  
Feature tool to display features related to a feature shown in form.  
function id="showRelatedFeatures"  
Parameters  
Parameter  Meaning  Mandatory  
relationType  Relation association used to find the to -be-displayed features  yes 
roleType  Relation role of the to -be-displayed  feature  yes 
Example:  
  <ber:featureTool name="Show existing inspections">  
    ...
    <ber:script>  
      <ber:function id=" showRelatedFeatures "/> IMPLEMENTATION GUIDE    VERSION 8.24 309 
 
 
  Implementation Guide        <ber:parameterArray>  
        <ber:parameter name="relationType" value="r_house_inspection"/>  
...
24.10  Feature presentation  
It’s possible to specify for a feature type:  
• color – attribute containing RGB color code. The color will be u sed for the icon representing the 
  ...
  <ber:featurePresentation>  
    ...
    <ber:featureType xlink:href="model.xml#ft_bbb">  
      <ber:attributeArray>  
        ...
        <ber:attribute id="color" xlink:href="mode l.xml#at_456/ca_010_color "/>  
      </ber:attributeArray>  
  </ber:featurePresentation>  
 
24.11  TELCO in LIDS Mobile  
List of feature types containing TELCO structur e considered  by LIDS Mobile is defined by specifying 
metadata group id in groupArray of mobile.xml . 310 24 – LIDS  MOBILE  
...
Example:  
  <ber:parameterArray>  
    <ber:parameter name="tenant" value="b usiness"/>  
 ...
 Chapter 2 5 
 Search Extension  
 IMPLEMENTATION GUIDE    VERSION 8.24 313 
 
 
  Implementation Guide  25. Search Extension  
LIDS Search extension provide s Elasticsearch functionality to standard LIDS and SAMO clients.  
Elasticsearch is a distributed, RESTful search and analytics engine. It provides scale and high 
availability.  Elasticsearch has the ability to be schema -less, which means that documents can be i ndexed 
...
xmlns:xli nk="http://www.w3.org/1999/xlink">  
  <ber:version>  
    <ber:metadata>1</ber:metadata>  
  ...
  <ber:esConnection>  
    ...
    <ber:indexPre fix>s__</ber:indexPrefix>  
  </ber:esConnection>  
    ...
    <ber:featureTypeIndexImage id=" fti_1" featureTypeId="*">  
      <ber:indexedAttributeArray>  
        <ber:indexedAttribute>  
          <ber:includeAttribute>*</ber:includeA ttribute>  
        </ber:indexedAttribute>  
      </ber:indexedAttributeArray>  
      <ber:indexGeometry>true</ber:indexGeometry>  
      <ber:indexGeometryForSearch>false</ber:indexGeometryForSearch>  314 25 – SEARCH EXTENSION  
 
 
 
Implementation Guide             <ber:geometryPrecision>5m</ber:geometryPrecision>  
    ...
    <ber:codelistIndexImage id="presetsExample" extends="  cdi_1" codelistId="cl_1">  
      <ber:indexedAttributeArray>  
        <ber:indexedAttribute>  
          ...
          <ber:fieldPreset>withoutdiacritics</ber:f ieldPreset>  
        </ber:indexedAttribute>  
        <ber:indexedAttribute>  
          <ber:includeAttribute>*</ber:includeAttribute>  
        </ber:indexedAttribute>  
  ...
  <ber:cronTriggerArray>  
    <ber:cronTrigger id="everyHour">  
      ...
      <ber:codelistsArray>  
        <ber:codelist codelistId="cl_ 1"/> 
        <ber:codelist codelistId="cl_ 2"/> IMPLEMENTATION GUIDE    VERSION 8.24 315 
 
 
  Implementation Guide        </ber:codel istsArray>  
    </ber:cronTrigger>  
  </ber:cronTriggerArray>  
</ber:search>  
...
• esClusterName  – name of Elastic search cluster  
...
Example:  
  <ber:esConnection>  
    ...
    <ber:esTransportAddressesArray>  
      <ber:esTranspor tAddress address="project.domain.com " port=" 10150"/> 
    </ber:esTransportAddressesArray>  
    <ber:indexPrefix>s__</ber:indexPrefix>  
  </ber:esConnection>  
Example:  Connection defined by parameters  
  <ber:esConnection>  
    <ber:esClusterName>${elasticClusterName}</b er:esClusterName>  
    <ber:esTransportAddressesArray>  
      <ber:esTransportAddress address="${elasticHost}" port="${elasticPort}"/>  
    </ber:esTransportAddressesArray>  
    <ber:indexPrefix>s__</ber:indexPrefix>  
...
It’s necessary to define, what data and how is indexed in Elasticsearch.  316 25 – SEARCH EXTENSION  
 
 
 
Implementation Guide       25.2.1  indexedFeaturesArray  
...
feature type ID s list or as group member definition). Graphic feature types from shared semantics group 
are not processed when evaluating the search quer ies. 
Example:  
  <ber:indexedFeaturesArray>  
    <ber:indexed Group groupId="gr_1"/> 
    <ber:indexed Group groupId="gr_2*"/> 
...
25.2.2  featureTypeIndexImage Array  
The way, how individual features of particular type are indexed is configured in 
featureTypeIndexImage . 
 Individual featureTypeIndexImages can be organized in a hie rarchy to allow inheriting common 
properties from parent images and defining specific properties at children images.  
...
particular attribute is defined in the query. Default is true  IMPLEMENTATION GUIDE    VERSION 8.24 317 
 
 
  Implementation Guide  • allFieldDateFormat – optional definition of the date attribute for adding into a llField index. If more 
definitions exist for the same attribute, the same value is added more times to the index. When no t 
...
value of the precission to higher number means lower precission, b ut also lower RAM consumption  
    ...
    <ber:indexedAttributeArray>  
      <ber:indexedAttribute>  
        ...
        <ber:allFieldDateFormatArray>  
          ...
          <ber:allFi eldDateFormat>dd.LLLL yyyy</ber:allFieldDateFormat>  
        </ber:allFieldDateFormatArray>  
      </ber:indexedAttribute>  
    </ber:indexedAttributeArray>  
    <ber:indexGeometry>false</ber:indexGeometry>  
    <ber:indexGeometryForSearch>false</ber:indexGeom etryForSearch>  
    <ber:allFieldTemplate>  
      <![CDATA[<#if at_ 12??>${at_ 12?number_to_date?string["dd.MM.yyyy"]}</#if>]]>  
    </ber:allFieldTemplate>  
  </ber:featureTypeIndexImage>  
  <ber:featureTypeIndexImage id="fti_2" extends="fti_1" featureTypeId="ft _abc*"> 
    <ber:indexedAttributeArray>  
      <ber:indexedAttribute>  
        <ber:includeAttribute>at_1</ber:includeAttribute>  
        <ber:addToAllField> false</ber:addToAllField>  
        <ber:fieldPreset>autocomplete</ber:fieldPreset>  
        <ber:fieldPreset>startswith</ber:fieldPreset>  318 25 – SEARCH EXTENSION  
 
 
 
Implementation Guide               <ber:fieldPreset>withoutdiacritics</ber:fieldPreset>  
      </ber:indexedAttribute>  
    </ber:indexedAttributeArray>  
    <ber:indexGeometry>true</ber:indexGeometry>  
    <ber:indexGeometryForSearc h>true</ber:indexGeometryForSearch>  
  ...
  <ber:featureTypeIndexImage id="fti_3" featureTypeId="ft_def">  
    <ber:indexedAttributeArray>  
      <ber:indexedAttribute>  
        ...
        <ber:exc ludeAttribute>atBar123</ber:excludeAttribute>  
      </ber:indexedAttribute>  
    </ber:indexedAttributeArray>  
    <ber:indexGeometry>false</ber:indexGeometry>  
...
Example:  
  <ber:indexedCodelistsAr ray> 
    <ber:indexed Group groupId="gr_1"/> 
    <ber:indexed Group groupId="gr_2"/> 
    <ber:indexedCodelist codelistId="cl_1"/>  
    <ber:indexedCodelist codelistId="cl_2"/>  IMPLEMENTATION GUIDE    VERSION 8.24 319 
 
 
  Implementation Guide      <ber:indexedCodelist codelistId="cl_abc*"/>  
  </ber:indexedCodelistsArray>  
25.3.2  code listIndexImage Array  
...
• id – identifier of index image  
• extends – identifier of parent image definition  
• codelist Id – particular codelist id or mask using wildcard  
• includeAttribute – particular attribute id or mask for defining set of attributes included in the  
definition  
• excludeAttribute – particular attribute id or mask for defining set of attributes excluded from the 
definition  
...
<ber:codelistIndexImageArray>  
  ...
  <ber:codelistIndexImage id="presetsExample" extends=" cdi_1" codelistId="cl_1">  
    <ber:indexedAttributeArray>  
      <ber:indexedAttribute>  
        <ber:includeAttribute>ca_1</ber:includeAttri bute> 
        <ber:fieldPreset>autocomplete</ber:fieldPreset>  
        <ber:fieldPreset>startswith</ber:fieldPreset>  
        <ber:fieldPreset>withoutdiacritics</ber:fieldPreset>  
      </ber:indexedAttribute>  
    </ber:indexedAttributeArray>  
  </ber:codelist IndexImage>  
  <ber:codelistIndexImage id="cdi_2" codelistId="cl_xyz">  
    <ber:indexedAttributeArray>  
      <ber:includeAttribute>ca 2</ber:includeAttribute>  
      <ber:includeAttribute>ca 3</ber:includeAttribute>  
      <ber:includeAttribute>ca4 </ber:include Attribute>  
      <ber:fieldPreset>withoutdiacritics</ber:fieldPreset>  320 25 – SEARCH EXTENSION  
 
 
 
Implementation Guide           </ber:indexedAttributeArray>  
  </ber:codelistIndexImage>  
</ber:codelistIndexImageArray>  
25.4 Updating Elasticsearch Index  
The consistency between LIDS and Elastic search is maintained automatically when working standard 
way with LIDS data.  
 
Situations whe n indexing  / reindexing must be  executed  on demand include : 
...
• by cron scheduler  
25.4.1  Reindexing scheduler configuration  
...
• authoredBy  – user login; the re -indexing is running under the identi ty of defined user  IMPLEMENTATION GUIDE    VERSION 8.24 321 
...
<ber:cronTriggerArray>  
  ...
  <ber:cronTrigger id="every Saturday At12"> 
...
# default ports after installation are 9200 and 9300. Can be changed following way:  322 25 – SEARCH EXTENSION  
 
 
 
Implementation Guide       http.port: 6200  
transport.tcp.port: 6300  
action.auto_create_index: 
.security*,.monitoring*,.watches,.triggered_watches,.watcher -history*,.ml*  
cluster.routing.allocation.disk.threshold_enabled: false  
 It’s a YAML file with specific syntax as described here:  
...
Software Foundation \Procrun 2.0 \elasticsearch -service-
x64\Parameters \Java\Options  
For more information, please, refer to Elasticsearch online documentation.  
25.6 Limitations  
Search extension  fully respects following se curity settings of LIDS:  
...
attribute shouldn’t be available for filtering, it ha s to be excluded from the Search extension indexing.  
 
 
  
   
 Chapter 26 
 Appendix A – True  Type Fonts 
in LIDS  System  
 IMPLEMENTATION GUIDE    VERSION 8.24 325 
 
 
  Implementation Guide  26. Appendix A - True Type Fonts in LIDS  System  
...
• Don't create new font as a n empt y project, copy it from correct/valid example (e.g. , 
Berit_CSN013411.ttf ) 
...
• Name of the font should be the same as the file  name  and should be the same for all platforms , see 
menu Format → Naming .. . 
 326 26 – APPENDIX A – TRUE TYPE FONTS IN LIDS  SYSTEM  
 
 
 
Implementation Guide       Example of naming properties window:  
...
• Don't forget check glyphs (see menu Font → Validate ). Frequently problems a re contour with wrong 
...
IMPLEMENTATION GUIDE    VERSION 8.24 327 
 
 
  Implementation Guide  26.4.1  Example of Glyph  
 
328 26 – APPENDIX A – TRUE TYPE FONTS IN LIDS  SYSTEM  
 
 
 
Implementation Guide       26.4.2  Glyph width  
...
completely ignores the right bea ring. 
...
• So if y ou want anchor point to be right part of the glyph:  
...
IMPLEMENTATION GUIDE    VERSION 8.24 329 
 
 
  Implementation Guide  26.4.3  Glyph height  
...
330 26 – APPENDIX A – TRUE TYPE FONTS IN LIDS  SYSTEM  
 
 
 
Implementation Guide        MicroStation and LID S Explorer can have problems with some char code ranges. It has been verified 
...
26.5 Tips for migration from LIDS 6 : 
...
The size and reference p oint for symbol in TTF must be modified according to this.  
...
IMPLEMENTATION GUIDE    VERSION 8.24 331 
...
332 26 – APPENDIX A – TRUE TYPE FONTS IN LIDS  SYSTEM  
 
 
 
Implementation Guide       26.6 Installation of TTF in LIDS  
There are two ways of installation TTF into LIDS.  
26.6.1  Server based installation  
...
clients up date their local copies of TTF  in the cache . 
26.6.2  Client based installation  
The font is only registered in resource.xml  without physical reference to TTF file. Then it must be 
installed into operating systems of all clients an d application server computers.  
 Fonts should be installed into operating system in a standard way. On Wind ows you can just copy fonts 
...
 If a font is installed both in the operating system and registered in the LIDS Applicatio n server resources, 
...
the PC. Otherwise , the change won’t be detected by LIDS Explorer.  IMPLEMENTATION GUIDE    VERSION 8.24 333 
 
 
  Implementation Guide  Example : Resource.xml  for client based installed TTF.  
 <ber:symbolFontArray>  
   <ber:symbolFont id="ber_LIDSdemo" name="ber_LIDSdemo">  
     <ber:formatArray>  
       <ber:format name="ttf"/>  
  ...
  334 26 – APPENDIX A – TRUE TYPE FONTS IN LIDS  SYSTEM  
...
Checks  IMPLEMENTATION GUIDE    VERSION 8.24 337 
 
 
  Implementation Guide  27. Appendix B – Advanced Data Checks  
...
27.1 Advanced Data Checks Principles  
• Advanced data checks are performed by means of database procedures which are stored in database 
package LIDS_CHECKS . This package con tains only basic set of predefined types of checks. Other 
...
condition.  
27.2 Advanced Data Checks Configuration i n LIDS Project  
...
• Creating ext-data-checks.xml  extended  metadata file.  338 27 – APPENDIX B – ADVANCED DATA CHECKS  
 
 
 
Implementation Guide       27.2.1  LIDS database schema configuration  
...
Column_name  Data type  Column description  
TASK_ID  NUMBER(10)  Task identifier (from APL_TASK_SEQ sequence); 
orimary key  
PROCEDURE_NAME  VARCHAR2(100)  Name of DB procedure which represents given 
check type  
START_DATE  DATE  Date, time when given check procedure was started  
END_DATE  DATE  Date, time when given check procedure was 
finished  
ERROR_ID  NUMBER(5)  Identifier of error if procedure was finidshed with 
error  
LOG  CLOB  Detailed logging information about procedure 
running  
RESULT  CLOB  List of result (erroneous) features  
...
For creating this table you can use script 02_CREATE_TABLE_Feature_types.sq l which is a part of 
instalation folder Extended_data_checks . 
FEATURE_TYPES table structure:  
Column_name  Data type  Column description  
FEATURE_CODE    VARCHAR2(100)  Feature type identifier (FTID)  
FEATURE_NAME    VARCHAR2(100)  Feature type name (not used in procedures)  
SEM_TABLE  VARCHAR2(30)  Semantic table name for given feature type  
FEA_TABLE  VARCHAR2(30)  Name of feature container table for given feature type  
(without _F, _GR suffix, e.g. GC_WAS)  
 
Only feature types which are used in advanced data checks need to be recorded in this table.  IMPLEMENTATION GUIDE    VERSION 8.24 339 
...
INSERT INT O feature_types (feature_code, feature_name, sem_table, fea_table) VALUES 
('ft_g_20100', 'WAW Leitung', 'WAS_LEITUNG', 'GC_WAS');  
27.2.2  LIDS settings configuration  
Hierarchy of advanced data check types and individual checks for specified data scope is configure d by 
...
advanced data check (e.g. „Find ing duplicate elements for 1 FTID“) should have defined 1 folder. It’s 
...
Name / Hierarc hy Setting type  Description of meaning  
LIDS Check settings  folder  Root folder for all advanced checks  
Find_duplicate_elems_1ftid  folder  Folder for Find_duplicate_elems_1ftid check type; 
parent folder = “LIDS Check setting” folder  340 27 – APPENDIX B – ADVANCED DATA CHECKS  
 
 
 
Implementation Guide       Name / Hierarc hy Setting type  Description of meaning  
Water category  setDefinition  It defines 1 specific check for 
“Find_duplicate_elems_1ftid”check type; data 
content is represented by all graphic feature types in 
Water category  
xxx setDefinition  Other specific check for given check type  
Find_duplicate_elems_nftid  folder  Folder for Find_duplicate_elems_Nftid check type; 
...
-- 
IMPLEMENTATION GUIDE    VERSION 8.24 341 
 
 
  Implementation Guide  INSERT INTO lids_settings (id, folder_id, type, user_access, name, description, 
...
Specific data check s are defined by particular setting with type „setdefinition“. This type of settings 
...
SET DEFINE OFF;  
INSERT INTO lids_setti ngs (id, folder_id, type, user_access, name, description, 
...
'&lt;ber:setDefinition version="1.2.0" xmln s:ber=”http://www.berit.com/ber ” 
...
'&lt;ber:featureType xlink:href="model.xml#ft_g_ 20200" /&gt;'||CHR(10)||  
...
COMMIT; 
SET DEFINR ON  
 Of course, LIDS set tings can be created via GUI in LIDS Explorer or LIDS Edit as well.  
27.2.3  LIDS p roject metadata configuration  
 Config.xml project configuration file adjustment  
Config.xml  file for given LIDS project must contain this definition in section <services><modules> . 
 342 27 – APPENDIX B – ADVANCED DATA CHECKS  
 
 
 
Implementation Guide       Example:  Config.xml . 
<module name="DbQuery">  
...
Service”>  
 ...
 Crea ting ext-data-checks.xml  extended metadat a file 
...
(LIDS_SETTIN GS.ID  attribute value).  
...
PropertyIsBetween . 
• ogc:PropertyName  – Attribute identifier ( ft_parcel_face /at_parcel_number ) or codelist attribute 
identifier ( ft_5060000 /at_5060002 /ca_00903 ). IMPLEMENTATION GUIDE    VERSION 8.24 343 
 
 
  Implementation Guide  27.3 Rules for Adding New Procedures to LIDS_CHECKS Package  
It’s possible to extend LIDS_CHECKS  package of additional procedures – data checks according to 
custom project requirements.  
Following rules must be observe d: 
...
lids_checks_API.sql  file in this form – with 2 mandatory input parameters and r eturning CLOB 
value, e.g.  
...
2. Description / purpose of fun ction should be attached to the function declaration as well.  
...
• Reading of spatial area from input parame ter by means of get_polygon  function.  
...
• Writing evaluated erroneous elements to out put CLOB variable.  
...
• FTID_id entifier1,FID_identifier1  
• FTID_id entifier2,FID_identifier2  
• FTID_id entifier3,FID_identifier3  
...
7. Each data check procedur e should have EXCEPTION  block for treatment of various unspecified 
procedure running errors; the SQL error code can be logged to protocol by means of modify_log  
subprocedure.  
8. Various auxiliary functions and subprocedures which are not directly called as da ta checks 
procedures may not be declared in API; it’s sufficient to place the only in package BODY.  344 27 – APPENDIX B – ADVANCED DATA CHECKS  
 
 
 
Implementation Guide       27.4 Verify whether JDBC Driver is Installed on JBoss Server  
27.4.1  Install JDBC driver  
Installation of JDBC driver on JBoss server is necessary for proper working of L IDS advanced data 
...
</module>  IMPLEMENTATION GUIDE    VERSION 8.24 345 
 
 
  Implementation Guide  
 Register the Driver Module  
1. Go to JBoss roo t directory and open standalone \configuration \standalone.xml . 
...
<driver name="Oracle JDBCDriver" module="com.oracle.db">  
  <driver-class>oracle.jdbc.driver.OracleDriver</driver -class> 
  <xa-datasource -class>oracle.jdbc.xa.client.OracleXADataSource</xa -datasource -
class> 
</driver>  
 
27.5 Using Advanced Data Checks in LIDS Client Applications  
Advan ced data checks can be used, i.e. user interface for it is available in LIDS Edit and LIDS Explorer 
...
you can find in user’s guide in the chapter Tools Menu  ->  Advanced data checks . 
  ...
  346 27 – APPENDIX B – ADVANCED DATA CHECKS  
...
Implementation Guide        
 ...
 IMPLEMENTATION GUIDE    VERSION 8.24 349 
 
 
  Implemen tation Guide  28. Appendix C – Backdrop Maps  
Backdrop maps are auxiliary graphic data lying under the graphic presentation of the active data – 
features. They can be both vectors and rasters (special case of raster backdrop is 
binaryRasterBackdropType ). 
28.1 Basic G oals and Principles  
...
• Structure and store parameters for tile cache and other tiles parameters are defined by LI DS project 
metadata.  
...
accessib le for LIDS Application Serv er. 
• This data are provided (downl oaded) by TMS or WMTS services.  
...
28.2 Raster Backdrop Maps  
28.2.1  WMS layer from external data source  
...
layer with id, name and format. You can also define maximum size  (width and heigh t) of the layer in 
...
... 
    <ber:rasterBackdropType id="bd_osm_world" name="OpenStreetMap -WMS"> 
      ...
      <ber:layerArray>  
        <ber:plainLayer id="lay_bd_osm_world_png" name="P NG Open Street Map WMS - 
world" format="png">  350 28 – APPENDIX C – BACKDROP MAPS 
 
 
 
Implementation Guide               <ber:maxSize width="512" height="512"/>  
...
... 
    <ber:wmsBackdropT ype version="1.1.1">  
...
to storage path of external data source. Other attributes refer to parameters of GetMap  request.  IMPLEMENTATION GUIDE    VERSION 8.24 351 
 
 
  Implemen tation Guide   If you work network infrastructure with proxy access there might occur a problem with external wms 
layers display (from the internet). You need to define access to the internet through proxy setting in 
LIDS Administration C onsole, see figure belo w. 
 
28.2.2  WMS layer from external datasource with defined hierarchy  
To speed up loading and displaying of backdrop map from external datasource, you can define its 
hierarchy and the backdrop map will be devided to tiles, w hich will than load separately.  
 Configu ring XML files  
Configuration of XML files is similar to chapter s 28.2.1.1 . First you need to define backdrop map in 
...
Browser.  
28.2.3  Raster layer with hierarchical structure from internal data source (basic 
...
352 28 – APPENDIX C – BACKDROP MAPS 
...
where decrease ratio  responds to steps of the hierarchy (higher means bigger steps, less con tinuity) and 
...
your needs, but rem ember to use logical names.  IMPLEMENTATION GUIDE    VERSION 8.24 353 
 
 
  Implemen tation Guide   
...
and its m ain tile layer (TIFF City Map).  
...
354 28 – APPENDIX C – BACKDROP MAPS 
 
 
 
Implementation Guide       
 Using LIDS Ultra Light Client  
This option is used when working with large amount of data, approximately more than 1000 file s, 
gigabytes of data.  
...
• All source TIFF files must have a georeferenc e. 
...
Generation of hier archical structure is done by LIDS Administration Console from the page Backdrops . 
Before start, you need to check if the parameters of generation – input (main tiled structure) and output 
(hierarchical structure) are set properly.The own generation of hie rarchical structure for given backdrop 
map can be activated by this icon:  
 
IMPLEMENTATION GUIDE    VERSION 8.24 355 
 
 
  Implemen tation Guide  The f ollowing dialog appears:  
 
 
Enter the file names of backdrop files, and run Rebuild . Raster JPEG files for the whole hierarchical 
structure should be created in your source f older ( C:\lids\lidsdemo \backdrops \City 
...
database, specifically in the table LIDS_BACKDROP . Followin g scripts are used to make sure if the 
...
You may want to display chosen backdrop map also in LIDS browser. You need to add its reference to 
included layers  root. 
356 28 – APPENDIX C – BACKDROP MAPS 
 
 
 
Implementation Guide       Example: Including raster layer with hierarchy in browser.xml . 
  <ber:includedLayers>  
... 
...
better to use hierarchical st ructure through the definition of TMS layer  in map-service.xml . Remember 
...
the Console, see fig ure below. Since you have already defined the backdrop hierarchy, defined tms layer 
automatically creates referention to it and no further changes are  needed.  
 
28.2.4  Raster layer with hierarchical structure from internal datasource 
(MicroStation formats hmr, cot, cit)  
This type of backdrop map is implemented much the same as the previous type, see chapter 28.2.2 . The 
only difference is format of source raster data file s, which server during the registration automatically 
...
These special MicroStation formats can be upl oaded to the server only with LIDS Edit license (valid for 
7.4.0 and older version branch).  
IMPLEMENTATION GUIDE    VERSION 8.24 357 
...
less continuity) and tile width  and height  (in pixels) respond to the level of det ail. Single tiled layers 
refer to different formats of source data, which can be combined together to create main tiled layer.  
Example: Definition of backdrop type  for raster layer with hierarchy in model.xml . 
  <ber:backdropTypeArray>  
... 
    <ber:rasterB ackdropType id="bd_lkw" name="Tests - raster">  
...
28.2.5  Historically versioned raster layer with hierarchical structure  
This type of backdrop map is used when you need to display same data from different time periods. The 
implementation is much the same as the previous type, see chapter 28.2.2 . In this chapter we only 
mention configuration of XML files and usage in LIDS Browser, which differs a little from the previous 
backdrop type.  
 Configuration of XML files  
First you need to define backdrop map in model.xml . Difference between unve rsioned and versioned 
...
format. You also nee d to define hierarchy for every hierarchical layer, where decrease ratio  responds to 
steps of the hierarchy (higher means bigger steps, less continuity) and tile width  and height  (in pixels) 
respond to the level of detail.  358 28 – APPENDIX C – BACKDROP MAPS 
 
 
 
Implementation Guide       Example: Definition of backdrop t ype for versioned raster layer with hierarchy in model.xml . 
  <ber:backdropTypeArray>  
... 
...
format="jpeg">  
          ...
          <ber:hierarchicalLayer id="lay_bd_02_jpeg_2013" name="JPEG Orthophoto - 
2013" format="jpeg" date="2013 -01-01 00:00:00">  
            <ber:hierarchy decreaseRatio="3" tileWidth="1000" tileHeight="1000"/>  
          </ber:hierarchicalLayer>  
          <ber:hierarchicalLayer id="lay_bd_02_jpeg_2014" name="JPEG Orthophoto - 
2014" format="jpeg" date="2014 -01-01 00:00:00">  
            <ber:hierarch y decreaseRatio="3" tileWidth="1000" tileHeight="1000"/>  
          </ber:hierarchicalLayer>  
        </ber:mainTiledLayer>  
      </ber:layerArray>  
    </ber:rasterBackdropType>  
... 
 ...
 Registration of source files  
Registration of source files runs similar as in chapter 28.2.3.3 . However to support historical versioning, 
...
IMPLEMENTATION GUIDE    VERSION 8.24 359 
...
layer in map-service.xml , see chapter 28.2.3.6 . However you need to define single tms layer for every 
hierarchical layer.  
...
... 
      ...
      <ber:tmsLayer id="tms_orthophoto_2014" sourceWMSLayer="bd_02" date="2014 -01-
01"/> 
      <ber:tmsLayer id="tms_orthophoto_2013" sourceWMSLayer="bd_02" da te="2013 -01-
01"/> 
... 
    </ber:tmsLayers>  
... 
  </ber:tms>  
28.3 Vector Backdrop Maps  
28.3.1  Backdrop layer from internal vector data  
It is also possible to create backdrop l ayer from internal vector data.  
 Configuration of XML files  
...
Example: Definition of backdrop type  for layer from internal vector data in model.xml . 
  <ber:backdropTypeArray>  
... 
    <ber:vectorBackdrop Type id="bd_01" name="Overview map">  
      <ber:description>More detailed vector backdrop</ber:description>  
      <ber:layerArray>  
        <ber:tiledLayer id="lay_bd_01_dgn" name="DGN overview map" format="dgn"/>  
        <ber:tiledLayer id="lay_bd_01_dwg" name="DWG overview map" format="dwg"/>  
      </ber:layerArray>  
    </ber:vectorBackdropType>  360 28 – APPENDIX C – BACKDROP MAPS 
...
Implementation Guide       ... 
 ...
 Registration  of source files  
After configuring model.xml , LIDS Administration Console au tomatically created folder hierarchy for 
expected source files. You can see it on page Backdrops . 
Since you are using internal data, you have to register them in the database, similar as in chapter 28.2.3.3 . 
...
28.3.2  TMS layer from LIDS project vector data  
This type of implementation is used when you want to create backdrop map fr om your own LIDS project 
...
You also need to define cache format  (for vector data usually png) and structure . Structure tag in map-
service.xml  contains reference to structure defined further.  IMPLEMENTATION GUIDE    VERSION 8.24 361 
 
 
  Implemen tation Guide  Example: Definition of tms la yer from project vector data in map-service.xml . 
  <ber:tms>  
... 
    <ber:tmsLayers>  
... 
...
maxY="5481900"/>          
        ...
        <ber:fixedScaleDenominators>  
          ...
          <ber:scaleDen ominator>5000</ber:scaleDenominator>  
...
... 362 28 – APPENDIX C – BACKDROP MAPS 
 
 
 
Implementation Guide             <ber:structure id="googleStruct" levels="22"  srs="EPSG:90 0913"> 
...
IMPLEMENTATION GUIDE    VERSION 8.24 363 
 
 
  Implemen tation Guide   The lower levels of the structure (level 8, 9, 10 from the picture) takes to much time to generate, so 
...
28.4 map-service.xml Structure and Possibilities  
...
<?xml version="1.0" encoding="UTF -8"?> 
<ber:modelMapService xm lns:ber="http://www.berit.com/ber" 
xmlns:xlink="http://www.w3.org/1999/xlink">  
  <ber:version>  
    <ber:metadata>1</ber:metadata>  
  ...
  <ber:wms>  
    <ber:contexts>  
      <ber:context id="RedliningSketch">  
        <ber:layer refId="cat_5070000">  
          <ber:dimension name="time" units="ISO8601" unitSymbol="H" default="2015 -
09-03" multipleValues="0" nearestValue="true" current="1" value="1900 -01-01/2100-
12-31/P1D"/>  
          <ber:dimension name="reference_time" default="2015 -09-03" 
units="ISO8601"/>  
        </ber:layer>  
        <ber:layer refId="cat_5080000" title="Sketch"/>  
      </ber:context>  364 28 – APPENDIX C – BACKDROP MAPS 
 
 
 
Implementation Guide             <ber:context id="UtilityNet">  
        <ber:layer refId="cat_5010000"/>  
  ...
  <ber:tms>  
    <ber:tmsLayers>  
      ...
      <ber:tmsLayer id="tms_detailed_plan_v8" sourceWMSLay er="bd_07_v8">  
        <ber:cache format="png">  
          <ber:structure refId="fixedDetailedStruct"/>  IMPLEMENTATION GUIDE    VERSION 8.24 365 
 
 
  Implemen tation Guide          </ber:cache>  
      </ber:tmsLayer>  
      ...
      <ber:tmsLayer id="tms_orthophoto_2014" sourceWMSLayer="bd_02" date="2014 -01-
01"/> 
      <ber:tmsLayer id="tms_orthophoto_2013" sourceWMSLayer="bd_02" date="2013 -01-
01"/> 
...
sourceWMSLayer="bd_osm_world_mundalis">  
        <ber:cache format="png">  
          <ber:structu re refId="fixedDetailedStruct"/>  
        </ber:cache>  
      </ber:tmsLayer>          
    </ber:tmsLayers>  
    <ber:tmsStructures>  
      ...
      <ber:structure id="fixedDetailedStruct">  
        ...
        <ber:fixedScaleDenomina tors> 
          ...
          <ber:scaleDenominator>2000</ber:scaleDenominator>  
          <ber:scaleDenominator>1000</ber:scaleDenominator>  
          <ber:scal eDenominator>500</ber:scaleDenominator>  
          <ber:scaleDenominator>200</ber:scaleDenominator>  366 28 – APPENDIX C – BACKDROP MAPS 
 
 
 
Implementation Guide                 <ber:scaleDenominator>100</ber:scaleDenominator>  
        </ber:fixedScaleDenominators>  
        <ber:gridOrigin x="3451605" y="5481732"/>  
      </ber:structure>  
...
</ber:mod elMapService>  
28.4.1  WMS Contexts and Services  
...
Optionally it’s possible to define fixed response URL which is not derived from the LIDS AS URL.  IMPLEMENTATION GUIDE    VERSION 8.24 367 
 
 
  Implemen tation Guide  <ber:wms>  
    <ber:contexts>  
      <ber:context id="  " url=""> 
        <ber:layer refId="cat egory_1"/> 
        <ber:layer refId="template_1 "/> 
        <ber:layer refId="backdrop _1"/> 
        ... 
28.4.2  TMS layers  
Using the TMS Layer LIDS AS will share ser vices through TMS specification. TMS services can be 
created from:  
• Category (defined  in model.xml ) 
• Setdefinition (setting from database)  
• Backdrop (defined in model.xml , using own backdrop hierarchy derived from decreaseRatio ) 
Layer definition can be limited to some date using the date="2017 -01-01". 
  <ber:tms>  
    <ber:tmsLayers>  
      <ber:tmsLayer id="tms_water" sourceWMSLayer="cat_5010000">  
        <ber:cache format="png">  
      ...
      ... 
      <ber:tmsLayer id="tms_orthophoto_2017" sourceWMSLayer="bd_02" date="2017 -01-
01"/> 
      ... 
...
maxY="5490500"/>  
      ...
      <ber:structure id="fixedDetailedStruct">  
        <ber:tileSize width="512" height="512"/>  368 28 – APPENDIX C – BACKDROP MAPS 
 
 
 
Implementation Guide               <ber:fixedScaleDenominators>  
          <ber:scaleDenominator>20000</ber:scaleDenominator>  
          <ber:scaleDenominator>10000</ber:scaleDenominator>  
          <ber:scaleDenominator>5000</ber:scaleDenominator>  
          <ber:scaleDenominator>2000</ber:scaleDenominator>  
      ...
      <ber:structure id="googleStruct" levels="22" srs="EPSG:900913">  
        <ber:tileSize width="256" height="256"/>  
        <ber:scaleDenomina tors min="282" max="591658711"/>  
        <ber:gridOrigin x=" -20037508.34" y="20037508.34"/>  
      </ber:structure>  
    </ber:tmsStructures>  
 
Every TMS structure can be limited to the defined “bounding Box” but it is not mandatory – the maximal 
...
the data til es will look like. In some cases small tiles are better for mobile devices and small displays  
...
define d levels as you want, just to have maybe not so much data/l evels on the server.  
...
The final TMS structure is available under the address: http://[lids -
as]/TileMapService/[tms_layer]/0/0/0.png  IMPLEMENTATION GUIDE    VERSION 8.24 369 
 
 
  Implemen tation Guide  List of TMS layers can be found in LIDS AS console, under Browser & Map services tab.  
...
results.  
  ...
  370 28 – APPENDIX C – BACKDROP MAPS 
...
Server Plugins  
 IMPLEMENTATION GUIDE    VERSION 8.24 373 
 
 
  Implementation Guide  29. Appendix D – Application Server  Plugins  
Plugins are exten sions to the  basic functionality od LIDS. Definition XML files for plugins/extensions  
are copied into this metadata directory : {project_folder} \lids-as\model \extensions . The plugins listeners 
are registered in config.xml  (see chapter 29.1). 
...
telco.xml  
29.1 Register Plugin on LIDS AS  
1. Copy your .xml file (for example relation -constraint s.xml) to model/ext ensions  directory.  
...
hence you must register it in both listener sections of config file.  
 374 29 – APPENDIX D – APPLICATION SERVER PLUGINS  
 
 
 
Implementation Guide       Example:  Config.xml  with registration of constraint listener.  
  <featureListeners>  
    ... 
...
• grouping ( groupDef inition ) – list of attributes in the given order.  
...
minValue  and maxValue are optional.  IMPLEMENTATION GUIDE    VERSION 8.24 375 
 
 
  Implementation Guide  List of predefined formats:  
...
PurpleSolidDataBar  
 376 29 – APPENDIX D – APPLICATION SERVER PLUGINS  
 
 
 
Implementation Guide       • topBottomRule  – selection of format  from predefined options (see list below) and parameter 
...
List of predefined formats for topBottomRule and cond itionRule : 
...
... 
      ...
      <ber:columnCustomization>  
        ...
        <ber:column field="at_5012103">  IMPLEMENTATION GUIDE    VERSION 8.24 377 
 
 
  Implementation Guide            <ber:conditionalFormat>  
...
... 
      <ber:columnCustomization>  
        <ber:col umn field="createDate" visible="true">  
          <ber:conditionalFormat>  
            <ber:colorScale>  
...
... 
 ...
 378 29 – APPENDIX D – APPLICATION SERVER PLUGINS  
 
 
 
Implementation Guide       29.3 db-procedures.xml  
...
serves mainly for operations specific for the given project and operations which can’t be perf ormed with 
...
</ber:dbProcedures>  IMPLEMENTATION GUIDE    VERSION 8.24 379 
 
 
  Implementation Guide  29.4 relation -constraint s.xml 
...
• Unique constraint – defines a child feature attribute for related features whose valu e must be unique.  
...
destinati on, only if the condition is met features can be related.  
The f ollowing elements are used in xml file, they are highlighted in example:  
• constraintArray  – represents a constraint collection; it contains one or more constraints.  
• uniqueConstraint  – element co ntains relation association reference, child role type reference and 
...
fulfilled.  
Example: Structure of rel ation -constraint s.xml.  
<?xml version="1.0" encoding="utf -8"?> 
...
segment count</description>  
      <relationAssoc xlink:href="model.xml#r_5040000_5010100"/>  380 29 – APPENDIX D – APPLICATION SERVER PLUGINS  
 
 
 
Implementation Guide             <roleType xlink:href="model.xml#rt_5040000_s"/>  
      <attribute xpath="at _5040002"/>  
...
'Unknown'</description>  
      <relationAssoc xlink:h ref="model.xml#r_5040000_5010100"/>  
      <roleType xlink:href="model.xml#rt_5010100_d"/>  
      <ogc:PropertyIsNotEqualTo>          <! -- choice of comparison or logic 
operator --> 
...
• derivedAttributeArray  – represents a collection o f attributes to derive.  
...
maxIntersectionL ength  and minIntersectionLength . 
Example: Structure of spatial -deriving.xml.  
<?xml version="1.0" encoding="utf -8"?> 
<spatialDeriving xmlns="http://www.berit.com/ber"  
                 xmlns:xlink="http://www.w3.org/1999/xlink">  IMPLEMENTATION GUIDE    VERSION 8.24 381 
 
 
  Implementation Guide   
  <!-- LIDSdemo project exam ple for spatial deriving plugin --> 
  <version>  
    <metadata>1</metadata>  
  ...
  <derivedAttributeArray>  
    <!-- Derived attribute: ft = Main pipeline segment, attr = Service OU - 
intersect ion --> 
...
dbName="OU_INTERSECT">  
  ...
  382 29 – APPENDIX D – APPLICATION SERVER PLUGINS  
...
Implementation Guide        
 ...
 Chapter 30 
 Appendix E – Resource s 
 IMPLEMENTATION GUIDE    VERSION 8.24 385 
 
 
  Implementation Guide  30. Appendix E – Resource s 
...
Explore r and LIDS Browser.  All additional resources needed for project are copied into this metadata 
...
xmlns:sld="http://www.opengis.net/sld" xmlns:ber="http://www.berit.com/ber">  
  ...
  <ber:lineStyleArray>  
  <ber:fillStyleArray>  
  <ber:custo mResourceArray type="dimStyle">  
...
30.1 dimstyle  
Resource is used for definition of  a dimension style s for LIDS Edit. Style is stored in DGN file (e.g. 
...
Example: Reference to a dimstyle file in resource.xml.  
  ...
  </ber:customResourceArray>  386 30 – APPENDIX E – RESOURCES  
 
 
 
Implementation Guide       30.2 exportshp  
Function "Export to SHP" is used for exporting feature types into ESRI Shapefile format. Output is ZIP 
archive containing DBF, SHP and SHX files - one s et for one fea ture type. In addition to identifications 
of exported features, all their attribute data and a text for text features, or a symbol for point features are 
saved to DBF files.  
A CSV file, which contains a table determining mapping between the column in the D BF file and feature 
...
30.2.1  Configuration of export  
...
names are defined, then they are generated automa tically during export.  
Example: attribute_name_mapping .csv 
at_4;District  
at_5;City 
at_6;Street  
30.3 fillstyle  
Resource is used for definition of a fill styles. Style is stored in XML file (e.g. fs_ demo .xml) in 
{project_folder} \lids-as\resources \fillstyle .  
There is also a reference to the particular style in resource.xml . IMPLEMENTATION GUIDE    VERSION 8.24 387 
 
 
  Implementation Guide  Example: Definition of a fill style.  
    ...
    <ber:lineSymbolizer>  
      <se:Stroke>  
        <se:GraphicStroke>  
          <se:Graphic>  
            <se:ExternalGraphic>  
              <se:OnlineResource xli nk:href="resource.xml#ls_2"/>  
              <se:Format/>  
            </se:ExternalGraphic>  
          </se:Graphic>  
        </se:GraphicStroke>  
  ...
  </ber:hatchPattern>  388 30 – APPENDIX E – RESOURCES  
 
 
 
Implementation Guide       Example: Reference to the style in a resource.xml.  
  <ber:fillStyleArray>  
...
30.4 icon 
Resource is used for storing icons  for tool.xml . Icons are stored in {project_folder} \lids-
as\resources \icon. They are divided into subfolder s according to the format  of the icon. This format  (for 
exam ple “bmp16x16”) is also specified in resource.xml .  
The f ollowing formats  can be used:  
_16x16  
_32x32  
_64x64  
bmp 
...
Example: Reference to the icon files in a resource.xml.  
  <ber:iconArray>  
    <ber:icon id="ic_e" name="ic_e">  
      <ber:formatArray>  
        <ber:format file="ic_e.bmp" name="bmp16x16"/>  IMPLEMENTATION GUIDE    VERSION 8.24 389 
 
 
  Implementation Guide        </ber:formatArray>  
    </ber:icon>   
... 
  </ber:iconArray>  
...
... 
    <ber:toolBox id="Tb_ele" name="Electro">  
      <ber:description>Tools for electro objects including cross -
sections</ber:description>  
...
30.5 importdgn  
Resource is used for importing CAD  data. Configuration XML files are stored in {project_folder} \lids-
as\resources \importdgn . All XML files are subsequently referenced in resource.xml.  
Example: XML file used for importing DGN data.  
<?xml version="1.0" encoding="utf -8"?>  
<ber:elementClassification xmlns="http://www.berit.com/ber" 
xmlns:ber="http://www.berit.com/ber">  
    <ber:attributesDefinitionArray>  390 30 – APPENDIX E – RESOURCES  
 
 
 
Implementation Guide             <ber:attributeDefinition id="attdef_01" name="Hydrant">  
        <ber:Description>Atributes - Hydrant</ber:Descriptio n> 
...
Example:  Reference to the import files in resource.xml.  
  <ber:customResourceArray type="importDGN">  
    ...
    <ber:customResource id="idgn_2" name="DGN Import var ious ftypes">  
      <ber:formatArray>  
...
resource.xml . IMPLEMENTATION GUIDE    VERSION 8.24 391 
 
 
  Implementation Guide  Example: XML file used for import.  
<?xml version="1.0" encoding="utf -8"?>  
<ber:elementClassification xmlns="ht tp://www.berit.com/ber" 
xmlns:ber="http://www.berit.com/ber">  
  <ber:attributesDefinitionArray>  
    <ber:attributeDefinition id="attdef_01" name="Definition 1">  
      <ber:Description>Attributes - contant</ber:Description>  
      <ber:attribute refId="at_50 80003" importType="constant">  
        <ber:parametersArray>  
...
Example: Reference to the import files in resource.xml.  
  ...
  </ber:customResourceArray>  392 30 – APPENDIX E – RESOURCES  
 
 
 
Implementation Guide       30.7 linestyle  
...
stored i n {project_folder} \lids-as\resources \linestyle \rsc or {project_folder} \lids-
as\resources \linestyle \lin 
• LIDS Mobile line styles are defined in JSON  
 
Individual l ine styles  definitions  have to be registered in resource.xml . Here the different formats are 
...
line style definition includes fo rmat name="rsc", but filena me attribute is missing. This tells to the 
...
1[ber_LIDSdemo2_120]</ber:stroke -dasharray>  
         <ber:segmentMod e>center</ber:segmentMode>  
        ...
        <ber:pointStroke id="ber_LIDSdemo2_123">  
          <ber:pointSymbolizer>  
            ...
            </se:Graphic>  IMPLEMENTATION GUIDE    VERSION 8.24 393 
 
 
  Implementation Guide              <ber:symbolHeight>2</ber:symbolHeight>  
            <ber:rotation type="relative">0</ber:rotation>  
          </ber:pointSymbolizer>  
        </ber:pointStroke>  
...
... 
    <ber:lineStyle id="KAN_POD_SPL" name="Waste water 7">  
      ...
      </ber:formatArray>  394 30 – APPENDIX E – RESOURCES  
 
 
 
Implementation Guide           </ber:lineStyle>  
... 
    <ber:lineStyle id=" STYLE_COMPLEX_3 " name=" Style complex 3 "> 
      <ber:formatArray>  
        <ber:format extResId=" STYLE_C3 " file="ls_linestyle3.xml" name="xml"/>  
        <!-- Following definiti on means the linestyle is included in the seed.dgn.  
          It can be used by:  
           - LIDS Edit  for displaying the data  
           - LIDS Edit / Explorer for DGN export --> 
        <ber:format extResId=" STYLE_COMPLEX_ 3" name="rsc"/>  
      </ber:formatArray>  
    </ber:lineStyle>  
... 
  </ber:lineStyleArray>  
...
id="r_ls_else">system.linestyle#solid</ber:value>  
                      </ber:elseMapItem>  IMPLEMENTATION GUIDE    VERSION 8.24 395 
...
30.7.1  Linestyle definitions for LIDS Mobile  
...
Line component definition consists of:  
• Used units (e.g. meters or pixels)  
• Optiona lly line component parts, so called dash -stroke, that defines size and relay of single parts. 
The parts can be of following types:  
• solid line  
• gap  
• symbol  
• text  396 30 – APPENDIX E – RESOURCES  
 
 
 
Implementation Guide       • Drawing mode  
...
length  
• Way how to solve situation when there is not enough spac e for a dash -stroke part or when some of 
size conditions (mentioned above) is not fulfilled. Possible ways:  
• render solid line  
• render nothing (default behavior)  
...
of used symbol)  
• symbol rotation - it can be of two types:  
• relative – rotation by specified angle towards th e given curve's segment  
• absolute – rotation towards global coordinate system  
 
Dash -stroke part definition – symbol:  
• definition of used symbol  
• symbol size - units are taken over from the parent line component  
• symbol indent (distance in x and y directio n from the dash -stroke part center towards the center 
of used symbol)  
• symbol rotation - it can be of two types:  
• relative – rotation by specified angle towards the given curve's segment  
• absolute – rotation towards global coordinate system  
...
Symbols and texts are not clipped within a dash -stroke part (i.e. they can exceed dash -stroke part size).  IMPLEMENTATION GUIDE    VERSION 8.24 397 
 
 
  Implementation Guide  Example: Definition of  line style s and its components  in JSON format – without symbols . 
 [ 
   { 
      "id" : "ls_1",  
      "unit" : "pixel",  
...
Implementation Guide             ] 
   ...
   { 
      ...
      "unit" : "pixel",  
      "dashArray" : [  
         { IMPLEMENTATION GUIDE    VERSION 8.24 399 
 
 
  Implementation Guide              "type" : "DASH",  
            "length" : 10  
         }, 
         { 
            "type" : "GAP",  
   ...
   { 
      ...
      "dashArray" : [  
         ...
         { 
            "type" : "GAP",  400 30 – APPENDIX E – RESOURCES  
 
 
 
Implementation Guide                   "length" : 5  
         } 
      ] 
   } 
] 
 
Example: Definition of  line style s and its components  in JSON format – with symbols . 
[ 
   { 
      "id" : "example_1",  
      "unit" : "metre",  
   ...
   { 
      ...
      "endSymbol" : {  
         ...
         "displacementX" : 1,  
         "displacementY" : -1 
      }, IMPLEMENTATION GUIDE    VERSION 8.24 401 
 
 
  Implementation Guide        "dashArray" : [  
         { 
            "type" : "DASH",  
            "length" : 18  
         }, 
...
rtf --> 402 30 – APPENDIX E – RESOURCES  
 
 
 
Implementation Guide           <ber:dynamicReport id="rp_1" name="Default Portrait" orientation="portrait" 
default="true">  
      <ber:description>Default Portrait Report Template</ber:description>  
      <ber:outputFormatArray>  
        <ber:outputFormat default="true">pdf</ber:outputFormat>  
      ...
      <ber:templateName>default.template.jrxml</ber:templateName>  
      <ber:attributeTemplat eName>default.template.xsl</ber:attributeTemplateName>  
    </ber:dynamicReport>  
... 
  </ber:reportArray>  
30.9 seeddgn  
Resource is used for storing seed files for LIDS Edit  (used for creating active system file and DGN 
export) and LIDS Explorer (used for DGN export) . Files are stored in {project_folder} \lids-
as\resources \seeddgn . Seed files are referenced in resource.xml.  
Example: Reference to the seed file in resource.xml.  
  ...
  </ber:customResourceArray>  IMPLEMENTATION GUIDE    VERSION 8.24 403 
 
 
  Implementation Guide  30.10  symbolfont  
Resource is used for storing information about symbol styles. Configuration XML files for LIDS 
Explorer and Browser (e.g.  ber_LIDSdemo77.ttf ) are stored in {project_folder} \lids-
as\resources \symbolfont \ttf. Style files for LIDS Edit (e.g. fonts .rsc) are stored in {project_folder} \lids-
as\resources \symbolfont \rsc. Fonts are subsequently referenced in resource.xml, thematization.xml  and 
model.xml . 
    ...
    <ber:featureTypeStyle id="st_t_rcont_1" name="Rack Contact">  
      ...
      <ber:combinedRule id="st_t_rcont_1_1" name="None">  
        <ber:pointSymbolizer>  
          <se:Graphic>  
            <se:ExternalGraphic>  
              <se:OnlineResource xlink:href="reso urce.xml#ber_LIDSdemo/35/center -
center"/>  
              <se:Format/>  
            </se:ExternalGraphic>  404 30 – APPENDIX E – RESOURCES  
...
... 
  </ber:graphicTypeArray>  IMPLEMENTATION GUIDE    VERSION 8.24 405 
 
 
  Implementation Guide  30.11  symbollibrary  
Resource is used for storing symbols for LIDS Edit. There are two types of library – cell library is stored 
in {project_folder} \lids-as\resources \symbollibrary \dgn, block library is stored in  {project_folder} \lids-
as\resources \symbollibrary \dwg. Libraries are subsequently referenced in resource.xml.  
...
Implementation Guide        
 ...
 Chapter 31 
 Appendix F – Security IMPLEMENTATION GUIDE    VERSION 8.24 409 
 
 
  Implementation Guide  31. Appendix F – Security 
31.1 Password rules  
